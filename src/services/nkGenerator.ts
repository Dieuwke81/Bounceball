...
export const generateNKSchedule = async (
  ...
  while (matchesRemaining > 0) {
    // Geef browser ruimte om te ademen tegen vastlopen
    await sleep(5);
    ...
    // Probeer verschillende rust-groepen
    for (let rAttempt = 0; rAttempt < 100; rAttempt++) {
        ...
        // Harde eis: High/Low mix voor reserves
        ...
        // We verhogen de balans-marge heel langzaam als het niet lukt
        let currentBalanceThreshold = 0.301;
        let foundMatches = false;

        for (let attempt = 0; attempt < 1000; attempt++) {
            if (attempt > 400) currentBalanceThreshold = 0.351;
            if (attempt > 800) currentBalanceThreshold = 0.451;
            ...
            for (let h = 0; h < hallsThisRound; h++) {
                ...
                if (Math.abs(avg1 - avg2) > currentBalanceThreshold) { tempValid = false; break; }
        ...
        if (foundMatches && roundPenalty < lowestPenalty) {
    ...
    // Afronden van de ronde (garantie dat er matches zijn)
    if (bestRoundMatches.length > 0) {
        ...
        // Noodstop als er echt niets gevonden wordt (voorkomt oneindige lus)
        console.error("Geen geldige match gevonden na alle pogingen.");
        break;
    }
  }
