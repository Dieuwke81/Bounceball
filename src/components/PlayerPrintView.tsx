...
const PlayerPrintView: React.FC<PlayerPrintViewProps> = ({ player, stats, trophies, players, seasonHistory, allTimeHistory, onClose }) => {
  ...
  useEffect(() => {
    // Iets langere wachttijd voor mobiel om te zorgen dat alles gerenderd is
    const printTimer = setTimeout(() => { window.print(); }, 800);
    // Sluit niet automatisch op mobiel, want dan ben je je print preview kwijt
    // We vertrouwen erop dat de gebruiker terugklikt of we sluiten na lange tijd
  ...
  return createPortal(
    <div className="print-portal hidden">
      <style>
        {`
          @media print {
            /* ESSENTIEEL VOOR IPHONE/ANDROID: Dwingt kleur af */
            * {
              -webkit-print-color-adjust: exact !important;
              print-color-adjust: exact !important;
            }
            
            /* VERBERG DE NORMALE APP */
            ...
            /* DE MAGIE: DWING EEN VASTE BREEDTE AF OP MOBIEL */
            .print-portal { 
                display: block !important; 
                position: relative !important; /* Relative werkt beter dan absolute op mobiel */
                top: 0; 
                left: 0; 
                width: 100% !important; 
                min-width: 750px !important; /* <--- DIT ZORGT DAT HET NIET IN ELKAAR KRIMPT */
                background: white; 
                color: black; 
        ...
        {trophies.length > 0 && (
            <div className="mb-8 break-inside-avoid">
                ...
                <div className="grid grid-cols-2 gap-3" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    {trophies.map(t => (
                        <div key={t.id} className="flex items-center border border-gray-200 p-2 rounded bg-gray-50 break-inside-avoid">
