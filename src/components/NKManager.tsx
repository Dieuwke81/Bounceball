...
const NKManager: React.FC<NKManagerProps> = ({ players, introPlayers = [], onClose }) => {
  ...
  // NIEUW: spelers die officieel deelnemen, maar fysiek vervangen worden
  const [reservePlayerIds, setReservePlayerIds] = useState<Set<number>>(new Set());
  ...
  const playerSchedules = useMemo(() => {
    ...
    const allSessionPlayers = session.rounds.flatMap(r =>
      r.matches.flatMap(m => [
        ...m.team1,
        ...m.team2,
        m.referee,
    ...
    const sessionPlayerMap = new Map<number, Player>();

    allSessionPlayers.forEach(p => {
      if (p) sessionPlayerMap.set(p.id, p);
    });

    activePlayerPool.forEach(p => {
      if (!sessionPlayerMap.has(p.id)) sessionPlayerMap.set(p.id, p);
    });
    ...
    // NIEUW:
    // Ook spelers die alleen als fysieke invaller voorkomen moeten op hun
    // individuele overzicht staan.
    const infillAssignments = (session as any).infillAssignments || [];

    infillAssignments.forEach((assignment: any) => {
      if (assignment?.substitutePlayerId != null) {
        participantIds.push(assignment.substitutePlayerId);
      }
    });

    const uniqueParticipantIds = Array.from(new Set(participantIds)).sort((a, b) => {
      const nameA = sessionPlayerMap.get(a)?.name || '';
      const nameB = sessionPlayerMap.get(b)?.name || '';
      return nameA.localeCompare(nameB);
    });

    return uniqueParticipantIds.map(id => {
      const p = sessionPlayerMap.get(id);

      const rounds = session.rounds.map(r => {
        // Eerst kijken of deze speler in deze ronde fysiek invaller is.
        const infill = infillAssignments.find(
          (assignment: any) =>
            assignment.roundNumber === r.roundNumber &&
            assignment.substitutePlayerId === id
        );

        if (infill) {
          return {
            round: r.roundNumber,
            hall: infill.hallName || '-',
            role: `INVAL ${infill.team || ''}`.trim(),
            startTime: (r as any).startTime || ''
          };
        }

        const match = r.matches.find(m =>
          [
            ...m.team1,
            ...m.team2,
            m.referee,
  ...
  // NIEUW:
  // Markeer een officieel geselecteerde speler als toernooi-reserve.
  // Maximaal 4 spelers.
  const handleToggleTournamentReserve = (playerId: number) => {
    if (!selectedPlayerIds.has(playerId)) return;

    const newReserves = new Set(reservePlayerIds);

    if (newReserves.has(playerId)) {
      newReserves.delete(playerId);
      setReservePlayerIds(newReserves);
      return;
    }

    if (newReserves.size >= 4) {
      alert('Je kunt maximaal 4 toernooi-reserves selecteren.');
      return;
    }

    newReserves.add(playerId);
    setReservePlayerIds(newReserves);
  };
  ...
  const handleTogglePlayer = (playerId: number) => {
    ...
    if (newSelected.has(playerId)) {
      ...
      // Als de speler niet meer deelnemer is, kan hij ook geen
      // toernooi-reserve meer zijn.
      if (reservePlayerIds.has(playerId)) {
        const newReserves = new Set(reservePlayerIds);
        newReserves.delete(playerId);
        setReservePlayerIds(newReserves);
      }
  ...
  const handleExportSession = () => {
    if (!session) return;
    try {
      const exportData = { session, nkType, playerSource, timestamp: new Date().toISOString() };
      const dataStr = JSON.stringify(exportData, null, 2);
      const blob = new Blob([dataStr], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `bounceball_export_${new Date().getTime()}.json`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      alert("Bestand gedownload! Stuur dit bestand naar je computer.");
    } catch (e) { alert("Export mislukt."); }
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        if (imported.session) {
          ...
          alert("Toernooi succesvol ingeladen!");
        }
      } catch (err) { alert("Fout bij inladen: Ongeldig bestand."); }
    };
    ...
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6 text-white font-black">
      <div className="flex justify-between items-center text-white">
        ...
        <div className="flex gap-2">
            <label className="cursor-pointer bg-blue-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-blue-500 transition-colors tracking-widest text-white">
                Importeer
      ...
      <div className="flex bg-gray-900 p-1 rounded-2xl w-fit mx-auto border border-gray-700">
        <button 
          onClick={() => { 
            setPlayerSource('database'); 
            setSelectedPlayerIds(new Set()); 
            setReservePlayerIds(new Set());
            setMinTeamRating(4.0); 
          }}
      ...
      {playerSource === 'database' && (
        <div className="flex bg-gray-900/50 p-1 rounded-2xl w-fit mx-auto border border-gray-700 mt-2 animate-fade-in">
          <button 
            onClick={() => setNkType('individual')} 
            className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase transition-all ${nkType === 'individual' ? 'bg-cyan-500 text-white shadow-lg' : 'text-gray-600'}`}
          >
            Individueel
          </button>
          <button 
            ...
            className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase transition-all ${nkType === 'fixed' ? 'bg-cyan-500 text-white shadow-lg' : 'text-gray-600'}`}
          >
            Vaste Teams
          </button>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-white font-black">
        <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-700 space-y-4 text-white font-black">
          <div>
            ...
            <input 
              ...
              className="w-full bg-gray-800 p-3 rounded-xl font-bold border border-gray-700 focus:border-amber-500 outline-none text-white font-black" 
            />
          </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {hallNames.map((name, i) => (
                <input 
                  ...
                  className="bg-gray-800 p-2 rounded-lg text-xs font-bold border border-gray-700 focus:border-amber-500 outline-none text-white font-black uppercase" 
          ...
          {playerSource === 'database' ? (
            <div>
              <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Min. Team Rating</span>
              <input 
                type="number" 
                step="0.01" 
                value={minTeamRating} 
                onFocus={(e) => e.target.select()} 
                onChange={e => setMinTeamRating(+e.target.value)} 
                className="w-full bg-gray-800 p-3 rounded-xl font-bold border border-gray-700 focus:border-amber-500 outline-none text-white font-black" 
              />
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <div className="flex gap-2 text-white font-black">
                {[2, 3, 4].map(n => (
                  <button 
                    key={n} 
                    onClick={() => setIntroPoolCount(n)} 
                    className={`flex-1 py-3 rounded-xl font-black border-2 ${introPoolCount === n ? 'bg-amber-500 border-amber-400 text-white' : 'bg-gray-800 border-gray-700 text-gray-500'}`}
                  >
                    {n} poules
                  </button>
                ))}
          ...
          <div className="flex gap-2 text-white font-black">
            {[4, 5].map(n => (
              <button 
                ...
                className={`flex-1 py-3 rounded-xl font-black border-2 ${playersPerTeam === n ? 'bg-amber-500 border-amber-400 text-white' : 'bg-gray-800 border-gray-700 text-gray-500'}`}
              >
                {n} vs {n}
              </button>
            ))}
          ...
          <textarea 
            ...
            placeholder="Plak WhatsApp lijst..." 
            className="w-full h-40 bg-gray-900 border border-gray-700 rounded-xl p-3 text-xs outline-none text-white font-black" 
          />

          <button 
            onClick={handleParseAttendance} 
            className="w-full py-3 bg-amber-500 text-white font-black rounded-xl uppercase text-xs hover:bg-amber-400 transition-all tracking-widest shadow-lg"
          >
            Verwerk Lijst
          </button>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4 text-white font-black">
          <div className="flex justify-between items-end text-white font-black">
            <h3 className="text-white font-bold uppercase text-xs tracking-widest">
              Deelnemers ({selectedPlayerIds.size})
            </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 max-h-48 overflow-y-auto p-2 bg-black/20 rounded-xl custom-scrollbar font-black">
            {activePlayerPool.map(p => {
                const rConfig = ratingColors.get(p.rating);
                const isSelected = selectedPlayerIds.has(p.id);
                const isTournamentReserve = reservePlayerIds.has(p.id);
                ...
                return (
                  ...
                  >
                    <button 
                      ...
                      className={`w-full p-2 pr-7 rounded-lg text-[10px] font-black border transition-all truncate 
                        ${isSelected 
                            ? (playerSource === 'intro' ? (rConfig?.bg || 'bg-amber-500') : 'bg-amber-500') + ' border-white text-white font-black' 
                            : 'bg-gray-800 border-gray-700'}`}
                    >
                      <span className={!isSelected && playerSource === 'intro' ? (rConfig?.text || 'text-white') : 'text-white'}>
                          {p.name}
                      </span>
                    </button>

                    {/* Reserve-knopje */}
                    {isSelected && (nkType === 'individual' || playerSource === 'intro') && (
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleToggleTournamentReserve(p.id);
                        }}
                        title={isTournamentReserve ? 'Toernooi-reserve verwijderen' : 'Markeer als toernooi-reserve'}
                        className={`absolute right-1 top-1/2 -translate-y-1/2 w-5 h-5 rounded-md text-[8px] font-black border transition-all ${
                          isTournamentReserve
                            ? 'bg-red-500 border-red-300 text-white shadow-lg'
                            : 'bg-gray-900/80 border-gray-600 text-gray-500 hover:border-amber-500 hover:text-amber-400'
                        }`}
                      >
                        R
                      </button>
          ...
          {(nkType === 'individual' || playerSource === 'intro') && (
            <div className="flex items-center justify-between bg-gray-900/60 border border-gray-700 rounded-xl px-4 py-3">
              <div>
                ...
                <div className="text-[9px] text-gray-600 uppercase font-bold mt-0.5">
                  Officieel deelnemer • fysiek vervangen door gelijke rating
                </div>
              </div>

              <div className={`text-sm font-black ${
                reservePlayerIds.size >= 4 ? 'text-amber-500' : 'text-white'
              }`}>
                {reservePlayerIds.size}/4
              </div>
            </div>
          )}
          ...
          {(nkType === 'individual' || playerSource === 'intro') ? (
            <div className="pt-4 border-t border-gray-700 space-y-4">
              <h3 className="text-white font-bold uppercase text-xs mb-3 text-amber-500 tracking-widest">
                1. Kies Toernooivorm:
              </h3>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {calculatedOptions.map(opt => (
                  <button 
                    ...
                    className={`p-4 rounded-2xl border-2 text-left transition-all ${selectedOption?.mpp === opt.mpp ? 'border-amber-500 bg-amber-500/20' : 'border-gray-700 bg-gray-800/40 hover:scale-[1.02]'}`}
                  >
                    <div className="text-xl font-black tracking-tighter text-white">
                      {opt.mpp} Wedstrijden p.p.
                    </div>
                    <div className="mt-1 text-[9px] font-bold text-gray-400 uppercase tracking-wider text-white font-black">
              ...
              {selectedOption && (
                <div className="bg-gray-900/50 p-6 rounded-3xl border-2 border-amber-500/30 animate-fade-in space-y-4">
                  <h3 className="text-white font-bold uppercase text-xs text-amber-500 tracking-widest text-center">
                    2. Voer Tijdschema in (Handmatig):
                  </h3>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-64 overflow-y-auto p-2 custom-scrollbar">
                    {manualTimes.map((time, idx) => (
                      <div key={idx} className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-white text-center font-black">
                        <div className="text-[10px] text-gray-500 font-black mb-2 uppercase">
                          Ronde {idx + 1}
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                          <input 
                            ...
                            value={time.start} 
                            placeholder="10:00" 
                            onChange={e => updateManualTime(idx, 'start', e.target.value)} 
                            className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black" 
                          />
                          ...
                          <input 
                            ...
                            value={time.end} 
                            placeholder="10:20" 
                            onChange={e => updateManualTime(idx, 'end', e.target.value)} 
                            className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black" 
                          />
                        </div>
                      </div>
                  ...
                  <button 
                    onClick={async () => {
                      ...
                      setProgressMsg("Balans optimaliseren..."); 
                      setErrorAnalysis(null);

                      try {
                        // Reserve-status wordt alleen op de kopie van de speler
                        // gezet die naar de generator gaat.
                        const p = activePlayerPool
                          .filter(x => selectedPlayerIds.has(x.id))
                          .map(x => ({
                            ...x,
                            isTournamentReserve: reservePlayerIds.has(x.id)
                          }));

                        const s = await generateNKSchedule(
                          p,
                          hallNames,
                          selectedOption.mpp,
                          playersPerTeam,
                    ...
                    className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl uppercase tracking-widest shadow-xl transition-all font-black"
                  >
                    Start Toernooi
                  </button>
                </div>
            ...
            <div className="pt-4 border-t border-gray-700 space-y-4 text-white font-black">
               <div className="bg-amber-500/10 p-6 rounded-3xl border-2 border-amber-500/30 text-center animate-fade-in space-y-2">
                <p className="text-sm font-black uppercase text-amber-500">
                  Vaste teams van {playersPerTeam}
                </p>
                <p className="text-[10px] text-gray-400 uppercase font-bold">
                  Iedereen speelt 1x tegen elk ander team (Round Robin).
                </p>

                {selectedPlayerIds.size > 0 && selectedPlayerIds.size % playersPerTeam !== 0 && (
                    <p className="text-red-500 text-[10px] mt-2 font-black italic">
                      LET OP: Selecteer nog {playersPerTeam - (selectedPlayerIds.size % playersPerTeam)} spelers (veelvoud van {playersPerTeam} nodig).
                    </p>
                )}
                ...
                {selectedPlayerIds.size > 0 && selectedPlayerIds.size % playersPerTeam === 0 && (
                    ...
                    className="mt-4 bg-amber-500 text-white px-8 py-3 rounded-2xl text-xs font-black uppercase shadow-lg hover:scale-105 transition-all font-black"
                    >
                      Bevestig Deelnemers
                    </button>
                )}
               ...
               {manualTimes.length > 0 && (
                <div className="bg-gray-900/50 p-6 rounded-3xl border-2 border-amber-500/30 animate-fade-in space-y-4 font-black">
                  <h3 className="text-white font-bold uppercase text-xs text-amber-500 tracking-widest text-center font-black">
                    Voer Tijdschema in (Handmatig):
                  </h3>

                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-64 overflow-y-auto p-2 custom-scrollbar font-black">
                    {manualTimes.map((time, idx) => (
                      <div key={idx} className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-white text-center font-black">
                        <div className="text-[10px] text-gray-500 font-black mb-2 uppercase">
                          Ronde {idx + 1}
                  ...
                  <button 
                    onClick={handleStartFixedNK} 
                    className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl uppercase tracking-widest shadow-xl transition-all font-black"
                  >
                    Start Vaste Teams Toernooi
                  </button>
                </div>
               )}
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 no-print text-white font-black">
          <div className="bg-gray-800 border-2 border-amber-500 rounded-3xl p-8 max-w-sm w-full space-y-6 shadow-2xl text-white font-black">
            ...
            <div className="space-y-3">
              <button onClick={() => handlePrintAction('overview')} className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black">
                Compleet Overzicht
              </button>
              <button onClick={() => handlePrintAction('halls')} className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black">
                Per Zaal
              </button>
              <button onClick={() => handlePrintAction('players')} className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black">
                Individuele Spelers
              </button>
            ...
            <button 
              ...
              className="w-full text-gray-500 font-bold uppercase text-[10px] hover:text-white transition-colors font-black"
            >
              Annuleren
            </button>
          </div>
      ...
      <div className="no-print bg-gray-800 p-6 rounded-3xl border-b-8 border-amber-500 shadow-2xl text-white space-y-6 font-black">
          ...
          <div className="flex bg-gray-900 p-1.5 rounded-2xl gap-1 w-full max-w-md font-black">
            {(['schedule', 'standings', 'analysis'] as const).map(t => (
              <button 
                ...
                className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${activeTab === t ? 'bg-amber-500 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300 font-black'}`}
              >
        ...
        <div className="flex justify-center gap-4 text-white font-black">
          <button 
            ...
            className="flex-1 max-w-[120px] bg-blue-600 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-blue-500 transition-colors text-white text-center font-black"
          >
            Exporteer
          </button>
          ...
          <button 
            ...
            className="flex-1 max-w-[120px] bg-gray-700 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-gray-600 transition-colors text-white text-center font-black"
          >
            Print
          </button>
          ...
          <button 
            onClick={() => { 
              if(confirm("NK Wissen?")) { 
                ...
                localStorage.removeItem('bounceball_nk_player_source'); 
                setSession(null); 
              } 
            }} 
            className="flex-1 max-w-[120px] bg-red-900/40 text-red-500 py-2.5 rounded-xl text-[10px] font-black uppercase border border-red-500/20 hover:bg-red-800 hover:text-white transition-all font-black"
          >
            Reset
          </button>
        </div>
      ...
      <div className="no-print space-y-8 text-white font-black">
        {activeTab === 'schedule' && (
          <>
            <input 
              type="text" 
              placeholder="Naam markeren..." 
              value={highlightName} 
              onChange={e => setHighlightName(e.target.value)} 
              className="w-full bg-gray-800 p-4 rounded-2xl text-white border border-gray-700 outline-none focus:ring-2 ring-green-500 transition-all font-black uppercase" 
            />
            
            {(session as any).isFixedTeams && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 animate-fade-in font-black">
                    <div className="col-span-full text-center text-gray-500 text-[10px] uppercase font-black mb-2 font-black">
                      Vaste Teams (Groepsgemiddelde):
                    </div>
            ...
            {! (session as any).isFixedTeams && (
                <div className="bg-gray-800/80 border-l-4 border-amber-500 p-4 rounded-r-2xl flex items-center gap-6 shadow-xl mb-4 text-white font-black">
                  <div className="flex-1 flex justify-between items-center text-white font-black">
                    <div>
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest leading-none font-black text-white">
                        Maximale rating verschil
                      </p>
                      ...
                      <div className="flex items-baseline gap-2 mt-1">
                        ...
                        <span className="text-[9px] font-bold text-gray-600 uppercase tracking-tight text-white font-black font-black">
                          (Beste van 250)
                        </span>
                      </div>
                    </div>
            ...
            {session.rounds.map((round, rIdx) => (
              <div key={rIdx} className="space-y-4 text-white font-black">
                <div className="flex items-center gap-4 text-white font-black">
                    <h3 className="text-xl font-black text-amber-500 uppercase italic border-l-4 border-amber-500 pl-4 tracking-tighter text-white">
                ...
                <div className="grid lg:grid-cols-2 gap-6 text-white font-black">
                  {round.matches.map((match, mIdx) => {
                    ...
                    if (isFixed) { 
                      if ((match as any).t1ReserveId) t1A = match.team1.filter(p => p.id !== (match as any).t1ReserveId); 
                      if ((match as any).t2ReserveId) t2A = match.team2.filter(p => p.id !== (match as any).t2ReserveId); 
                    }

                    const avg1 = t1A.reduce((s, p) => s + p.rating, 0) / t1A.length; 
                    const avg2 = t2A.reduce((s, p) => s + p.rating, 0) / t2A.length;

                    return (
                      <div 
                        key={match.id} 
                        className={`match-card bg-gray-800 rounded-2xl border-2 ${match.isPlayed ? 'border-green-500/50 shadow-lg shadow-green-500/5 font-black' : 'border-gray-700'} overflow-hidden text-white font-black`}
                      >
                        <div className="bg-gray-900/50 p-3 flex justify-between text-[10px] font-black uppercase text-gray-500 tracking-widest text-white font-black">
                          <span>
                            📍 ZAAL <span className="text-red-500 text-sm ml-1 font-black uppercase text-white font-black">
                              {match.hallName}
                            </span>
                          </span>

                          {isFixed ? (
                             <span className="text-cyan-400 font-black font-black">
                               Vaste Teams
                             </span>
                          ) : (
                             <span className={`px-2 py-0.5 rounded transition-all ${isHighlighted(match.referee?.name || '') ? 'bg-green-500 text-white font-black scale-110 shadow-lg font-black' : 'text-pink-400 font-black'}`}>
                        ...
                        <div className="p-5 flex justify-between items-stretch gap-4 text-white font-black">
                          <div className="flex-1 space-y-1 text-left text-white font-black font-black">
                            <div className="text-[9px] text-blue-400 font-black uppercase mb-2 tracking-widest text-white font-black">
                              {(match as any).team1Name || 'Team Blauw'}
                            </div>
                            ...
                            <div className="text-[9px] text-gray-500 mt-2 font-black font-black">
                              GEM: {avg1.toFixed(2)}
                            </div>
                          </div>
                          ...
                          <div className="flex flex-col items-center justify-center gap-3 text-white font-black font-black">
                            <div className="flex items-center gap-2 text-white font-black font-black">
                              <input 
                                ...
                                onFocus={(e) => e.target.select()} 
                                onChange={e => { 
                                  const n = JSON.parse(JSON.stringify(session)); 
                                  n.rounds[rIdx].matches[mIdx].team1Score = +e.target.value; 
                                  n.rounds[rIdx].matches[mIdx].isPlayed = true; 
                                  setSession(n); 
                                }} 
                              ...
                              <input 
                                ...
                                onChange={e => { 
                                  const n = JSON.parse(JSON.stringify(session)); 
                                  n.rounds[rIdx].matches[mIdx].team2Score = +e.target.value; 
                                  n.rounds[rIdx].matches[mIdx].isPlayed = true; 
                                  setSession(n); 
                                }} 
                                className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border border-gray-700 text-white outline-none focus:border-amber-500 font-black" 
                            ...
                            <div className="flex flex-col items-center gap-1 text-white font-black font-black">
                              <button 
                                ...
                                className={`text-[8px] font-black px-3 py-1.5 rounded-lg transition-all ${match.isPlayed ? 'bg-green-600 text-white shadow-lg font-black' : 'bg-gray-700 text-gray-400 hover:text-white font-black'}`}
                              >
                                {match.isPlayed ? 'VERWERKT' : 'OPSLAAN'}
                              </button>

                              <button 
                                onClick={() => { 
                                  const n = JSON.parse(JSON.stringify(session)); 
                                  n.rounds[rIdx].matches[mIdx].isPlayed = false; 
                                  n.rounds[rIdx].matches[mIdx].team1Score = 0; 
                                  n.rounds[rIdx].matches[mIdx].team2Score = 0; 
                                  setSession(n); 
                                }} 
                                className="text-[8px] text-red-500 font-black mt-1 uppercase font-black"
                              >
                                Reset
                              </button>

                              <div className="text-[8px] text-gray-500 font-bold uppercase tracking-tighter mt-1 font-black">
                                Verschil: {Math.abs(avg1 - avg2).toFixed(2)}
                              </div>
                            </div>
                          </div>

                          <div className="flex-1 space-y-1 text-right text-white font-black font-black">
                            <div className="text-[9px] text-amber-400 font-black uppercase mb-2 tracking-widest text-white font-black">
                              {(match as any).team2Name || 'Team Geel'}
                            </div>
                        ...
                        {!isFixed && (
                          <div className="p-2.5 bg-gray-900/30 border-t border-gray-700 flex justify-center gap-8 text-[9px] font-black uppercase text-white font-black font-black font-black font-black font-black font-black font-black font-black">
                            <span className={`px-2 rounded transition-all border-l-2 ${playerSource === 'intro' ? (ratingColors.get(match.subHigh?.rating)?.border || 'border-transparent') : 'border-transparent'} ${isHighlighted(match.subHigh?.name || '') ? 'bg-green-500 text-white font-black font-black' : 'text-pink-400 font-black'}`}>
                              Res 1: {match.subHigh?.name}
                            </span>

                            <span className={`px-2 rounded transition-all border-l-2 ${playerSource === 'intro' ? (ratingColors.get(match.subLow?.rating)?.border || 'border-transparent') : 'border-transparent'} ${isHighlighted(match.subLow?.name || '') ? 'bg-green-500 text-white font-black font-black' : 'text-pink-400 font-black'}`}>
          ...
          <div className="bg-gray-800 rounded-3xl shadow-2xl border border-gray-700 overflow-hidden text-white animate-fade-in font-black text-center font-black">
            <table className="w-full text-left font-black text-white uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">
              <thead className="bg-gray-900 text-gray-400 text-[10px] font-black tracking-widest uppercase font-black">
                <tr>
                  <th className="p-5 w-12 text-center text-white font-black uppercase font-black">#</th>
                  <th className="p-5 text-white font-black uppercase text-left font-black font-black font-black font-black">
                    {(session as any).isFixedTeams ? 'Team' : 'Speler'}
                  </th>
                  <th className="p-5 text-center text-white font-black uppercase font-black font-black">W</th>
                  <th className="p-5 text-center text-white font-black uppercase font-black font-black font-black">DS</th>
                  <th className="p-5 text-center text-white font-black uppercase font-black">PTN</th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-gray-700/50 uppercase text-white font-black font-black">
                {currentStandings.map((entry, idx) => (
                  <tr 
                    key={idx} 
                    className={`transition-colors ${idx < 3 ? 'bg-amber-500/5' : 'hover:bg-gray-700/30'} text-white font-black font-black`}
                  >
                    <td className="p-5 text-center font-black text-amber-500 uppercase font-black">
                      {idx + 1}
                    </td>

                    <td className="p-5 font-bold text-sm tracking-tight text-white font-black uppercase text-left font-black font-black">
                      {(entry as any).name || (entry as any).playerName}
                    </td>

                    <td className="p-5 text-center text-gray-400 font-mono text-xs text-white font-black uppercase font-black font-black">
                      {(entry as any).matchesPlayed}
                    </td>

                    <td className={`p-5 text-center font-black font-mono text-xs ${entry.goalDifference > 0 ? 'text-green-500' : entry.goalDifference < 0 ? 'text-red-500' : 'text-gray-500'}`}>
                      {entry.goalDifference > 0 ? `+${entry.goalDifference}` : entry.goalDifference}
                    </td>

                    <td className="p-5 text-center font-black font-black font-black font-black font-black font-black font-black font-black">
                      <span className="bg-gray-900 text-amber-400 px-4 py-1.5 rounded-full font-black text-sm shadow-inner border border-amber-500/20 font-black font-black font-black">
                        {entry.points}
                      </span>
                    </td>
                  </tr>
        ...
        {activeTab === 'analysis' && (
          <div className="space-y-4 animate-fade-in text-white no-print font-black font-black font-black">
            <input 
              type="text" 
              placeholder="Duo's filteren..." 
              value={searchTerm} 
              onChange={e => setSearchTerm(e.target.value)} 
              className="w-full bg-gray-800 border-2 border-gray-700 rounded-2xl p-4 text-sm outline-none focus:border-amber-500 transition-all text-white font-black uppercase font-black" 
            />
            ...
            <div className="bg-gray-800 rounded-3xl border border-gray-700 overflow-hidden max-h-[600px] overflow-y-auto uppercase custom-scrollbar text-white text-center font-black font-black">
              <table className="w-full text-left text-white font-black uppercase text-center font-black">
                <thead className="bg-gray-900 text-gray-400 text-[10px] uppercase font-black sticky top-0 z-20 text-white font-black uppercase">
                  <tr>
                    <th className="p-5 text-white font-black uppercase text-left font-black">Duo</th>
                    <th className="p-5 text-center text-white font-black uppercase font-black font-black">Samen</th>
                    <th className="p-5 text-center text-white font-black uppercase font-black font-black">Tegen</th>
                    <th className="p-5 text-center text-white font-black uppercase font-black">Totaal</th>
                  </tr>
                </thead>

                <tbody className="divide-y divide-gray-700/50 text-white font-black uppercase font-black font-black">
                  {coOpData
                    .filter(d => d.p1.toLowerCase().includes(searchTerm.toLowerCase()) || d.p2.toLowerCase().includes(searchTerm.toLowerCase()))
                    .map((pair, i) => {
                    const total = pair.together + pair.against;

                    let totalColor = "bg-green-500 text-white font-black font-black"; 
                    if (total === 0) totalColor = "bg-transparent text-gray-600 font-black font-black";
                    else if (total === totalRankings.highest) totalColor = "bg-red-500 text-white shadow-lg font-black font-black";
                    else if (total === totalRankings.second) totalColor = "bg-orange-500 text-white font-black font-black";
                    else if (total === totalRankings.third) totalColor = "bg-yellow-500 text-black font-black font-black";
                    else if (total === totalRankings.fourth) totalColor = "bg-yellow-200 text-black font-bold font-black";

                    return (
                      <tr 
                        key={i} 
                        className="hover:bg-gray-700/20 transition-colors text-white font-black uppercase font-black font-black"
                      >
                        <td className="p-5 text-xs font-bold tracking-tight text-left uppercase text-white font-black font-black">
                          {pair.p1} + {pair.p2}
                        </td>

                        <td className="p-5 text-center text-xs text-gray-400 font-mono text-white font-black uppercase font-black font-black">
                          {pair.together}x
                        </td>
                        ...
                        <td className="p-5 text-center font-black font-black font-black font-black">
