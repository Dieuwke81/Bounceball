...
const NKManager: React.FC<NKManagerProps> = ({ players, onClose }) => {
  ...
  const updateMatch = (roundIdx: number, matchIdx: number, updates: Partial<any>) => {
    ...
    Object.assign(newSession.rounds[roundIdx].matches[matchIdx], updates);
    ...
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-white">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-end"><h3 className="text-white font-bold uppercase text-xs">Spelers ({selectedPlayerIds.size})</h3><button onClick={() => setSelectedPlayerIds(new Set())} className="text-[10px] text-red-500 font-bold uppercase underline">Wis alles</button></div>
          ...
          {selectedPlayerIds.size > 0 && (
            <div className="pt-4 border-t border-gray-700">
              <h3 className="text-white font-bold uppercase text-xs mb-3 text-amber-500">Mogelijke Schema's:</h3>
      ...
      <div className="no-print flex justify-between items-center bg-gray-800 p-4 rounded-2xl border-b-4 border-amber-500 shadow-xl text-white">
        ...
        <div className="flex bg-gray-900 p-1 rounded-xl gap-1">
          {(['schedule', 'standings', 'analysis'] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-2 rounded-lg font-bold text-[10px] uppercase transition-all ${activeTab === t ? 'bg-amber-500 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}>{t === 'schedule' ? 'Schema' : t === 'standings' ? 'Stand' : 'Social'}</button>
          ))}
        </div>
          ...
          <>
            ...
            {session.rounds.map((round, rIdx) => (
              <div key={rIdx} className="space-y-4">
                <h3 className="text-xl font-black text-amber-500 uppercase italic border-l-4 border-amber-500 pl-4">Ronde {round.roundNumber}</h3>
                      ...
                      <div key={match.id} className={`match-card bg-gray-800 rounded-2xl border-2 ${match.isPlayed ? 'border-green-500/50 shadow-lg shadow-green-500/5' : 'border-gray-700'} overflow-hidden`}>
                        ...
                        <div className="p-5 flex justify-between items-stretch gap-4 text-white">
                          <div className="flex-1 space-y-1">
                            <div className="text-[9px] text-blue-400 font-black uppercase mb-2">Team Blauw</div>
                          ...
                          <div className="no-print flex flex-col items-center justify-center gap-3">
                            <div className="flex items-center gap-2">
                              <input type="number" value={match.team1Score} onChange={e => updateMatch(rIdx, mIdx, { team1Score: +e.target.value, isPlayed: true })} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border-2 border-gray-700 text-white outline-none focus:border-amber-500" />
                              <span className="text-gray-600 font-bold">-</span>
                              <input type="number" value={match.team2Score} onChange={e => updateMatch(rIdx, mIdx, { team2Score: +e.target.value, isPlayed: true })} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border-2 border-gray-700 text-white outline-none focus:border-amber-500" />
                            </div>
                            <button onClick={() => updateMatch(rIdx, mIdx, { isPlayed: !match.isPlayed })} className={`text-[8px] font-black px-3 py-1.5 rounded-lg transition-all ${match.isPlayed ? 'bg-green-600 text-white shadow-lg' : 'bg-gray-700 text-gray-400 hover:text-white'}`}>{match.isPlayed ? 'HERSTEL' : 'OPSLAAN'}</button>
                          </div>
                          <div className="flex-1 space-y-1 text-right">
                            <div className="text-[9px] text-amber-400 font-black uppercase mb-2">Team Geel</div>
