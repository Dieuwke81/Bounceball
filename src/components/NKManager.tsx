...
const NKManager: React.FC<NKManagerProps> = ({ players, introPlayers = [], onClose }) => {
  ...
  // NIEUW: geselecteerde spelerpositie voor handmatig wisselen
  const [swapSelection, setSwapSelection] = useState<SwapPosition | null>(null);
  ...
  const currentStandings = useMemo(() => {
    ...
    if (isFixed) {
        ...
        session.rounds.forEach(r => r.matches.forEach(m => {
            ...
            const t1Id = (session as any).fixedTeams.findIndex(
              (ft: any) => ft.players[0].id === m.team1[0].id
            );
            const t2Id = (session as any).fixedTeams.findIndex(
              (ft: any) => ft.players[0].id === m.team2[0].id
            );
        ...
        session.rounds.forEach(r => r.matches.forEach(m => {
          ...
          const p1 = m.team1Score > m.team2Score
            ? 3
            : m.team1Score === m.team2Score
              ? 1
              : 0;

          const p2 = m.team2Score > m.team1Score
            ? 3
            : m.team1Score === m.team2Score
              ? 1
              : 0;
  ...
  const coOpData = useMemo(() => {
    ...
    for (let i = 0; i < participantIds.length; i++) {
      for (let j = i + 1; j < participantIds.length; j++) {
        ...
        pairsMap.set(key, {
          p1: activePlayerPool.find(p => p.id === id1)?.name || '?',
          p2: activePlayerPool.find(p => p.id === id2)?.name || '?',
          together: 0,
          against: 0
        });
      }
    ...
    session.rounds.forEach(r => r.matches.forEach(m => {
      const add = (
        ...
        const d = pairsMap.get([id1, id2].sort().join('-'));
        if (d) d[type]++;
      };

      m.team1.forEach((p, idx) =>
        m.team1.slice(idx + 1).forEach(p2 =>
          add(p.id, p2.id, 'together')
        )
      );

      m.team2.forEach((p, idx) =>
        m.team2.slice(idx + 1).forEach(p2 =>
          add(p.id, p2.id, 'together')
        )
      );
      ...
      m.team1.forEach(p =>
        m.team2.forEach(p2 =>
          add(p.id, p2.id, 'against')
        )
      );
    }));

    return Array.from(pairsMap.values()).sort(
      (a, b) => (b.together + b.against) - (a.together + a.against)
    );
  }, [session, activePlayerPool]);

  const totalRankings = useMemo(() => {
    const totals = coOpData.map(d => d.together + d.against);
    const uniqueSorted = Array.from(new Set(totals))
      .filter(n => n > 0)
      .sort((a, b) => b - a);
  ...
  const playerSchedules = useMemo(() => {
    ...
    const participantIds = Array.from(
      new Set(
        session.rounds.flatMap(r =>
          r.matches.flatMap(m =>
            [...m.team1, ...m.team2, m.referee, m.subHigh, m.subLow]
              .filter(p => p)
              .map(p => p!.id)
          )
        )
  ...
  // NIEUW:
  // Wisselt uitsluitend de twee aangeklikte spelerposities.
  // Alleen binnen dezelfde ronde, zodat een speler niet tegelijk
  // speler + reserve/scheidsrechter kan worden.
  const handlePlayerSwap = (
    ...
    // Vaste teams mogen niet worden aangepast via deze functie.
    if ((session as any).isFixedTeams) return;

    // Wisselen kan alleen vóór de eerste gespeelde wedstrijd.
    const tournamentStarted = session.rounds.some(r =>
    ...
    const clickedPosition: SwapPosition = {
      rIdx,
      mIdx,
      team,
      playerIdx
    ...
    // Eerste speler aanklikken
    if (!swapSelection) {
      setSwapSelection(clickedPosition);
      return;
    }

    // Nogmaals dezelfde speler/positie aanklikken = selectie annuleren
    if (
      swapSelection.rIdx === rIdx &&
      swapSelection.mIdx === mIdx &&
    ...
    // Alleen binnen dezelfde ronde wisselen.
    if (swapSelection.rIdx !== rIdx) {
      alert('Je kunt alleen twee spelers binnen dezelfde ronde wisselen.');
      setSwapSelection(null);
      return;
    }
    ...
    const firstMatch = newS.rounds[swapSelection.rIdx].matches[swapSelection.mIdx];
    const secondMatch = newS.rounds[rIdx].matches[mIdx];

    const firstTeam =
      swapSelection.team === 1
        ? firstMatch.team1
        : firstMatch.team2;
    ...
    const firstPlayer = firstTeam[swapSelection.playerIdx];

    firstTeam[swapSelection.playerIdx] = secondTeam[playerIdx];
    secondTeam[playerIdx] = firstPlayer;
  ...
  const handleStartFixedNK = async () => {
    ...
    setProgressMsg("Optimaliseren (Best of 1.000.000)...");

    try {
      const p = activePlayerPool.filter(
        x => selectedPlayerIds.has(x.id)
      );

      const s = await generateFixedNKSchedule(
        p,
        hallNames,
        playersPerTeam,
        "NK Vaste Teams",
  ...
  const updateFixedTeamName = (
    ...
    newS.rounds.forEach((r: any) =>
      r.matches.forEach((m: any) => {
        if (
          m.team1[0].id ===
          newS.fixedTeams[teamIdx].players[0].id
        ) {
          m.team1Name = newName;
        }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (imported.session) {
          ...
          if (imported.nkType) {
            setNkType(imported.nkType);
          }

          if (imported.playerSource) {
            setPlayerSource(imported.playerSource);
          }
        ...
        alert("Fout bij inladen: Ongeldig bestand.");
      }
    };
  ...
  if (isGenerating) return (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-white text-center font-black">
      ...
      <p className="text-gray-500 text-xs mt-2 uppercase font-bold animate-pulse tracking-widest text-center">
  ...
  const isHighlighted = (name: string) =>
    highlightName &&
    name.toLowerCase().includes(highlightName.toLowerCase());
    ...
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6 text-white font-black">
      <div className="flex justify-between items-center text-white">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onClose}
            className="bg-gray-700 px-4 py-2 rounded-xl text-[10px] font-bold uppercase hover:bg-gray-600 transition-colors tracking-widest text-white"
          >
            Terug
          </button>
        </div>
      ...
      {errorAnalysis && (
        <div className="bg-red-500/10 border-2 border-red-500/50 p-4 rounded-2xl text-white">
          <h3 className="text-red-500 font-black uppercase text-sm">
            Let op: Analyse Mislukking
          </h3>
          <p className="text-gray-300 text-[10px] mt-1 leading-relaxed whitespace-pre-line">
            {errorAnalysis}
          </p>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-white font-black">
        <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-700 space-y-4 text-white font-black">
          ...
          <div className="space-y-2 text-white font-black">
            <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest text-white">
              Zaalnamen
            </span>
              ...
              <div className="flex gap-2 text-white font-black">
                {[2, 3, 4].map(n => (
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-black border-2 ${
                      introPoolCount === n
                ...
                {calculatedOptions.map(opt => (
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 text-left transition-all ${
                      selectedOption?.mpp === opt.mpp
                        ? 'border-amber-500 bg-amber-500/20'
                        : 'border-gray-700 bg-gray-800/40 hover:scale-[1.02]'
                    }`}
                  >
                ...
                <div className="bg-gray-900/50 p-6 rounded-3xl border-2 border-amber-500/30 animate-fade-in space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-64 overflow-y-auto p-2 custom-scrollbar">
                    {manualTimes.map((time, idx) => (
                      <div
                        key={idx}
                        className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-white text-center font-black"
                      >
            ...
            <div className="pt-4 border-t border-gray-700 space-y-4 text-white font-black">
              <div className="bg-amber-500/10 p-6 rounded-3xl border-2 border-amber-500/30 text-center animate-fade-in space-y-2">
                ...
                {selectedPlayerIds.size > 0 &&
                  selectedPlayerIds.size % playersPerTeam !== 0 && (
                ...
                {selectedPlayerIds.size > 0 &&
                  selectedPlayerIds.size % playersPerTeam === 0 && (
                    <button
                      onClick={() => {
                        const numTeams =
                          selectedPlayerIds.size / playersPerTeam;
      ...
      {printMenuOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 no-print text-white font-black">
          <div className="bg-gray-800 border-2 border-amber-500 rounded-3xl p-8 max-w-sm w-full space-y-6 shadow-2xl text-white font-black">
            <h3 className="text-2xl font-black uppercase italic text-center text-white">
              Print Menu
            </h3>

            <div className="space-y-3">
              <button
                onClick={() => handlePrintAction('overview')}
              ...
              <button
                onClick={() => handlePrintAction('halls')}
                className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black"
              >
                Per Zaal
              </button>
              ...
              <button
                onClick={() => handlePrintAction('players')}
                className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black"
              >
                Individuele Spelers
              </button>
            ...
            {(['schedule', 'standings', 'analysis'] as const).map(t => (
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${
                  activeTab === t
                    ? 'bg-amber-500 text-white shadow-lg'
                    : 'text-gray-500 hover:text-gray-300 font-black'
                }`}
              >
                {
                  t === 'schedule'
      ...
      <div className="no-print space-y-8 text-white font-black">
        {activeTab === 'schedule' && (
          <>
            ...
            {/* NIEUW: instructie voor handmatig wisselen */}
              ...
              <div className={`p-4 rounded-2xl border-2 transition-all ${
                ...
                {swapSelection ? (
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-[10px] text-green-400 font-black uppercase tracking-widest">
                        Speler geselecteerd
                      </div>
                      <div className="text-xs text-white font-black uppercase mt-1">
                        Klik nu op de tweede speler om de twee plekken te wisselen.
                      </div>
                    </div>

                    <button
                      onClick={() => setSwapSelection(null)}
                      className="shrink-0 px-4 py-2 bg-gray-700 hover:bg-red-600 rounded-xl text-[9px] font-black uppercase transition-all"
                    >
                      Annuleren
                    </button>
                  </div>
                  ...
                  <div>
                    <div className="text-[10px] text-amber-500 font-black uppercase tracking-widest">
                      Spelers handmatig wisselen
                    </div>
                    <div className="text-[10px] text-gray-400 font-black uppercase mt-1">
                      Klik twee spelers in dezelfde ronde aan. Alleen die twee velden worden omgewisseld.
                    </div>
                  </div>
                )}
              </div>
            ...
            {(session as any).isFixedTeams && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 animate-fade-in font-black">
                ...
                {(session as any).fixedTeams.map((team: any, i: number) => (
                  <div
                    key={i}
                    className="bg-gray-800 p-3 rounded-2xl border border-gray-700 shadow-lg font-black"
                  >
                    <div className="flex justify-between items-center mb-1 font-black">
                      <input 
                        ...
                        value={team.name} 
                        onChange={e =>
                          updateFixedTeamName(i, e.target.value)
                        } 
                    ...
                    <div className="flex items-baseline gap-2 mt-1">
                      <span
                        className={`text-3xl font-black italic tracking-tighter ${
                          maxTournamentDiff > 0.4
                            ? 'text-red-500'
                            : maxTournamentDiff > 0.25
                              ? 'text-amber-500'
                              : 'text-green-500'
                        }`}
                      >
              ...
              <div key={rIdx} className="space-y-4 text-white font-black">
                ...
                <div className="grid lg:grid-cols-2 gap-6 text-white font-black">
                  {round.matches.map((match, mIdx) => {
                    ...
                    if (isFixed) {
                      if ((match as any).t1ReserveId) {
                        t1A = match.team1.filter(
                          p => p.id !== (match as any).t1ReserveId
                        );
                      }

                      if ((match as any).t2ReserveId) {
                        t2A = match.team2.filter(
                          p => p.id !== (match as any).t2ReserveId
                        );
                      }
                    }

                    const avg1 =
                      t1A.reduce((s, p) => s + p.rating, 0) /
                      t1A.length;

                    const avg2 =
                      t2A.reduce((s, p) => s + p.rating, 0) /
                      t2A.length;

                    return (
                      <div
                        key={match.id}
                        className={`match-card bg-gray-800 rounded-2xl border-2 ${
                          match.isPlayed
                            ? 'border-green-500/50 shadow-lg shadow-green-500/5 font-black'
                            : 'border-gray-700'
                      ...
                      >
                        <div className="bg-gray-900/50 p-3 flex justify-between text-[10px] font-black uppercase text-gray-500 tracking-widest text-white font-black">
                          <span>
                            📍 ZAAL{' '}
                            <span className="text-red-500 text-sm ml-1 font-black uppercase text-white font-black">
                              {match.hallName}
                            </span>
                          </span>
                          ...
                          {isFixed ? (
                            ...
                            <span
                              className={`px-2 py-0.5 rounded transition-all ${
                                isHighlighted(match.referee?.name || '')
                                  ? 'bg-green-500 text-white font-black scale-110 shadow-lg font-black'
                        ...
                        <div className="p-5 flex justify-between items-stretch gap-4 text-white font-black">
                          <div className="flex-1 space-y-1 text-left text-white font-black font-black">
                            ...
                            {match.team1.map((p, playerIdx) => {
                              const rColor = ratingColors.get(p.rating);
                              const isR =
                                (match as any).t1ReserveId === p.id;
                              ...
                              return (
                                <div
                                  ...
                                  className={`text-sm uppercase font-bold border-l-2 pl-2 ${
                                    playerSource === 'intro'
                                      ? (rColor?.border || 'border-transparent')
                                      : 'border-transparent'
                                  } transition-all ${
                                    isHighlighted(p.name)
                                      ? 'bg-green-500 text-white px-1 rounded-sm scale-105 shadow-md font-black'
                                      : 'font-black'
                                  } ${
                                    isFixed
                                      ? 'cursor-pointer hover:text-amber-500 font-black'
                                      : 'cursor-pointer hover:text-amber-400 font-black'
                                  } ${
                                    isR
                                      ? 'line-through opacity-40 grayscale font-black'
                                      : 'font-black'
                                  } ${
                                    isSwapSelected
                                      ? 'bg-green-500 text-white rounded-md px-2 py-1 scale-[1.02] shadow-lg border-green-400'
                                      : ''
                                  }`}
                                >
                                  {p.name}
                          ...
                          <div className="flex flex-col items-center justify-center gap-3 text-white font-black font-black">
                            <div className="flex items-center gap-2 text-white font-black font-black">
                              <input
                                ...
                                onChange={e => {
                                  ...
                                  n.rounds[rIdx].matches[mIdx].team1Score =
                                    +e.target.value;

                                  n.rounds[rIdx].matches[mIdx].isPlayed =
                                    true;
                              ...
                              <span className="text-gray-600 font-black font-black">
                                -
                              </span>
                            ...
                            <div className="flex flex-col items-center gap-1 text-white font-black font-black">
                              <button
                                ...
                                className={`text-[8px] font-black px-3 py-1.5 rounded-lg transition-all ${
                                  match.isPlayed
                                    ? 'bg-green-600 text-white shadow-lg font-black'
                                    : 'bg-gray-700 text-gray-400 hover:text-white font-black'
                                }`}
                              >
                                {match.isPlayed
                                  ? 'VERWERKT'
                          ...
                          <div className="flex-1 space-y-1 text-right text-white font-black font-black">
                            ...
                            {match.team2.map((p, playerIdx) => {
                              const rColor = ratingColors.get(p.rating);
                              const isR =
                                (match as any).t2ReserveId === p.id;

                              const isSwapSelected =
                                swapSelection?.rIdx === rIdx &&
                                swapSelection?.mIdx === mIdx &&
                                swapSelection?.team === 2 &&
                                swapSelection?.playerIdx === playerIdx;
                              ...
                              return (
                                <div
                                  ...
                                  className={`text-sm uppercase font-bold border-r-2 pr-2 ${
                                    playerSource === 'intro'
                                      ? (rColor?.border || 'border-transparent')
                                      : 'border-transparent'
                                  } transition-all ${
                        ...
                        {!isFixed && (
                          <div className="p-2.5 bg-gray-900/30 border-t border-gray-700 flex justify-center gap-8 text-[9px] font-black uppercase text-white font-black font-black font-black font-black">
                            <span
                              className={`px-2 rounded transition-all border-l-2 ${
                                playerSource === 'intro'
                                  ? (
                                      ratingColors.get(
                                        match.subHigh?.rating
                                      )?.border ||
                                      'border-transparent'
                                    )
                                  : 'border-transparent'
                                ...
                                isHighlighted(
                                  ...
                                  ? 'bg-green-500 text-white font-black font-black'
                                  : 'text-pink-400 font-black'
                              }`}
                            >
                ...
                {/* Bestaande weergave van spelers die deze ronde niets doen */}
                {(round as any).restingPlayers?.length > 0 && (
                  <div className="bg-gray-900/60 border border-gray-700 rounded-2xl p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(round as any).restingPlayers.map(
                        (player: Player) => (
                          <span
                            key={player.id}
                            className={`px-3 py-1.5 rounded-lg bg-gray-800 border border-gray-700 text-xs uppercase font-black ${
                              isHighlighted(player.name)
          ...
          <div className="bg-gray-800 rounded-3xl shadow-2xl border border-gray-700 overflow-hidden text-white animate-fade-in font-black text-center font-black">
            <table className="w-full text-left font-black text-white uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black">
              <thead className="bg-gray-900 text-gray-400 text-[10px] font-black tracking-widest uppercase font-black">
                <tr>
                  <th className="p-5 w-12 text-center text-white font-black uppercase font-black">
                    #
                  </th>

                  <th className="p-5 text-white font-black uppercase text-left font-black font-black font-black">
                    {(session as any).isFixedTeams
                      ? 'Team'
                      : 'Speler'}
                  </th>

                  <th className="p-5 text-center text-white font-black uppercase font-black">
                    W
                  </th>

                  <th className="p-5 text-center text-white font-black uppercase font-black font-black font-black">
                    DS
                  </th>
              ...
              <tbody className="divide-y divide-gray-700/50 uppercase text-white font-black font-black">
                {currentStandings.map((entry, idx) => (
                  ...
                  >
                    ...
                    <td className="p-5 text-center text-gray-400 font-mono text-xs text-white font-black uppercase font-black font-black font-black">
                      {(entry as any).matchesPlayed}
                    </td>

                    <td
                      className={`p-5 text-center font-black font-mono text-xs ${
                    ...
                    <td className="p-5 text-center font-black font-black font-black font-black font-black">
            ...
            <div className="bg-gray-800 rounded-3xl border border-gray-700 overflow-hidden max-h-[600px] overflow-y-auto uppercase custom-scrollbar text-white text-center font-black font-black font-black">
              <table className="w-full text-left text-white font-black uppercase text-center font-black">
                <thead className="bg-gray-900 text-gray-400 text-[10px] uppercase font-black sticky top-0 z-20 text-white font-black uppercase">
                  <tr>
                    <th className="p-5 text-white font-black uppercase text-left font-black">
                      Duo
                    </th>

                    <th className="p-5 text-center text-white font-black uppercase font-black font-black">
                      Samen
                    </th>
                ...
                <tbody className="divide-y divide-gray-700/50 text-white font-black uppercase font-black font-black">
                  {coOpData
                    ...
                    .map((pair, i) => {
                      ...
                      if (total === 0) {
                        totalColor =
                          "bg-transparent text-gray-600 font-black font-black";
                      }
                      else if (
                        total === totalRankings.highest
                      ) {
                        totalColor =
                          "bg-red-500 text-white shadow-lg font-black font-black";
                      }
                      else if (
                        total === totalRankings.second
                      ) {
                        totalColor =
                          "bg-orange-500 text-white font-black";
                      }
                      else if (
                        total === totalRankings.third
                      ) {
                        totalColor =
                          "bg-yellow-500 text-black font-black";
                      }
                      else if (
                        total === totalRankings.fourth
                      ) {
                        totalColor =
                          "bg-yellow-200 text-black font-bold font-black";
                      }
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-5 text-center font-black font-black font-black font-black font-black">
                            <span
                              className={`px-3 py-1 rounded-lg text-[10px] font-black transition-all ${totalColor}`}
                            >
                              {total}x
                            </span>
                          </td>
