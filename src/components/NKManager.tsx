...
const NKManager: React.FC<NKManagerProps> = ({ players, introPlayers = [], onClose }) => {
  ...
  if (!session) return (
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6 text-white font-black">
      <div className="flex justify-between items-center text-white font-black font-black">
        <div className="flex items-center gap-4 text-white font-black font-black">
          <TrophyIcon className="w-10 h-10 text-amber-500" />
          <h2 className="text-3xl font-black uppercase italic tracking-tighter text-white font-black font-black">NK Setup</h2>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-white font-black text-white font-black">
        ...
        <div className="lg:col-span-2 space-y-4 text-white font-black">
          <div className="flex justify-between items-end text-white font-black text-white font-black font-black"><h3 className="text-white font-bold uppercase text-xs tracking-widest">Deelnemers ({selectedPlayerIds.size})</h3><button onClick={() => setSelectedPlayerIds(new Set())} className="text-[10px] text-red-500 font-bold uppercase underline">Wis alles</button></div>
          ...
          {selectedPlayerIds.size > 0 && (
            <div className="pt-4 border-t border-gray-700 space-y-4 text-white font-black">
              ...
              <div className="grid sm:grid-cols-2 gap-3 text-white font-black font-black">
                {calculatedOptions.map(opt => (
                  <button key={opt.mpp} onClick={() => handleSelectOption(opt)} className={`p-4 rounded-2xl border-2 text-left transition-all ${selectedOption?.mpp === opt.mpp ? 'border-amber-500 bg-amber-500/20' : 'border-gray-700 bg-gray-800/40 hover:scale-[1.02]'} text-white font-black`}>
                    <div className="text-xl font-black tracking-tighter text-white font-black">{opt.mpp} Wedstrijden p.p.</div>
              ...
              {selectedOption && (
                <div className="bg-gray-900/50 p-6 rounded-3xl border-2 border-amber-500/30 animate-fade-in space-y-4 text-white font-black">
                  <h3 className="text-white font-bold uppercase text-xs text-amber-500 tracking-widest text-center">2. Voer Tijdschema in (Handmatig):</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-64 overflow-y-auto p-2 custom-scrollbar text-white font-black text-white font-black font-black font-black">
                    {manualTimes.map((time, idx) => (
                      <div key={idx} className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-white text-center font-black">
                        ...
                        <div className="flex items-center gap-1 text-white font-black">
                          <input type="text" value={time.start} placeholder="10:00" onChange={e => updateManualTime(idx, 'start', e.target.value)} className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black font-black" />
                          <span className="text-gray-600 font-black font-black">-</span>
                          <input type="text" value={time.end} placeholder="10:20" onChange={e => updateManualTime(idx, 'end', e.target.value)} className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black font-black" />
                        </div>
                      </div>
                    ))}
                  </div>
                  <button onClick={async () => {
                    ...
                    }} className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl uppercase tracking-widest shadow-xl transition-all font-black font-black font-black">Start Toernooi</button>
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="space-y-6 pb-20 text-white font-black font-black text-white font-black">
      <style>{`
        @media print {
      ...
      {printMenuOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 no-print text-white font-black">
          <div className="bg-gray-800 border-2 border-amber-500 rounded-3xl p-8 max-w-sm w-full space-y-6 shadow-2xl text-white font-black text-white font-black">
            <h3 className="text-2xl font-black uppercase italic text-center text-white font-black">Print Menu</h3>
            ...
            <button onClick={() => setPrintMenuOpen(false)} className="w-full text-gray-500 font-bold uppercase text-[10px] hover:text-white transition-colors">Annuleren</button>
          </div>
        </div>
      )}

      <div className="no-print bg-gray-800 p-6 rounded-3xl border-b-8 border-amber-500 shadow-2xl text-white space-y-6 font-black font-black text-white font-black">
        <h2 className="text-4xl sm:text-6xl font-black italic uppercase tracking-tighter text-center font-black">NK Manager</h2>
        <div className="flex justify-center text-white font-black text-white font-black font-black">
          <div className="flex bg-gray-900 p-1.5 rounded-2xl gap-1 w-full max-w-md text-white font-black font-black">
        ...
        <div className="flex justify-center gap-4 text-white font-black">
          ...
          <button onClick={() => { if(confirm("NK Wissen?")) { localStorage.removeItem('bounceball_nk_session'); setSession(null); } }} className="flex-1 max-w-[120px] bg-red-900/40 text-red-500 py-2.5 rounded-xl text-[10px] font-black uppercase border border-red-500/20 hover:bg-red-800 hover:text-white transition-all font-black text-white font-black">Reset</button>
        </div>
      </div>

      <div className="no-print space-y-8 text-white font-black text-white font-black font-black">
        {activeTab === 'schedule' && (
          <>
            ...
            <div className="bg-gray-800/80 border-l-4 border-amber-500 p-4 rounded-r-2xl flex items-center gap-6 shadow-xl mb-4 text-white font-black">
              <div className="flex-1 flex justify-between items-center text-white font-black">
                <div>
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest leading-none text-white font-black">Maximale rating verschil</p>
                  <div className="flex items-baseline gap-2 mt-1 text-white font-black font-black">
                ...
                <div className="hidden sm:block text-right max-w-[220px] text-white">
                  <p className="text-[9px] font-bold text-gray-400 uppercase leading-tight italic font-black text-white font-black">
            ...
            {session.rounds.map((round, rIdx) => (
              <div key={rIdx} className="space-y-4 text-white font-black">
                <div className="flex items-center gap-4 text-white font-black">
                    <h3 className="text-xl font-black text-amber-500 uppercase italic border-l-4 border-amber-500 pl-4 tracking-tighter text-white font-black font-black">Ronde {round.roundNumber}</h3>
                    {(round as any).startTime && (
                        <span className="bg-gray-800 px-3 py-1 rounded-lg border border-gray-700 text-xs font-black text-gray-400 text-white font-black font-black">
                            {(round as any).startTime} - {(round as any).endTime}
                        </span>
                    )}
                </div>
                <div className="grid lg:grid-cols-2 gap-6 text-white font-black text-white font-black">
                    ...
                    return (
                      <div key={match.id} className={`match-card bg-gray-800 rounded-2xl border-2 ${match.isPlayed ? 'border-green-500/50 shadow-lg shadow-green-500/5' : 'border-gray-700'} overflow-hidden text-white font-black`}>
                        <div className="bg-gray-900/50 p-3 flex justify-between text-[10px] font-black uppercase text-gray-500 tracking-widest text-white font-black">
                          ...
                          <span className={`px-2 py-0.5 rounded transition-all ${isHighlighted(match.referee?.name || '') ? 'bg-green-500 text-white font-black scale-110 shadow-lg' : 'text-pink-400 font-black'}`}>Ref: {match.referee?.name}</span>
                        </div>
                        <div className="p-5 flex justify-between items-stretch gap-4 text-white font-black">
                          <div className="flex-1 space-y-1 text-left text-white font-black font-black">
                            <div className="text-[9px] text-blue-400 font-black uppercase mb-2 tracking-widest text-white font-black">Team Blauw</div>
                            {match.team1.map(p => {
                            ...
                            <div className="text-[9px] text-gray-500 mt-2 font-black text-left font-black">GEM: {avg1.toFixed(2)}</div>
                          </div>
                          <div className="flex flex-col items-center justify-center gap-3 text-white font-black">
                            <div className="flex items-center gap-2 text-white font-black">
                              <input type="number" value={match.team1Score} onFocus={(e) => e.target.select()} onChange={e => {
                                  ...
                                  setSession(newS);
                                }} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border-2 border-gray-700 text-white outline-none focus:border-amber-500 text-white font-black font-black" />
                              <span className="text-gray-600 font-black font-black">-</span>
                            ...
                            <div className="flex flex-col items-center gap-1 text-white font-black">
                              ...
                              <div className="text-[8px] text-gray-500 font-bold uppercase tracking-tighter text-white font-black font-black">Verschil: {Math.abs(avg1 - avg2).toFixed(2)}</div>
                            </div>
                          </div>
                          <div className="flex-1 space-y-1 text-right text-white font-black">
                            <div className="text-[9px] text-amber-400 font-black uppercase mb-2 tracking-widest text-right text-white font-black">Team Geel</div>
                            {match.team2.map(p => {
                                const rColor = ratingColors.get(p.rating);
                                return (
                            ...
                            <div className="text-[9px] text-gray-500 mt-2 font-black text-right text-white font-black">GEM: {avg2.toFixed(2)}</div>
                          </div>
                        </div>
                        <div className="p-2.5 bg-gray-900/30 border-t border-gray-700 flex justify-center gap-8 text-[9px] font-black uppercase text-white font-black">
        ...
        {/* STANDINGS & ANALYSIS TABELLEN BLIJVEN ONGEWIJZIGD */}
      </div>
