...
const NKManager: React.FC<NKManagerProps> = ({ players, onClose }) => {
  ...
  if (isGenerating) return (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-white text-center">
      ...
      <p className="text-gray-500 text-[10px] uppercase font-bold tracking-widest mt-2">Balans: 0.3 | Kwaliteit: 4.0+</p>
    </div>
  );
  ...
  if (!session) return (
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-white">
        <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-700 space-y-4 shadow-inner">
          <div><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Zalen</span><input type="number" value={hallsCount} onChange={e => setHallsCount(+e.target.value)} className="w-full bg-gray-800 p-3 rounded-xl font-bold border border-gray-700 outline-none focus:ring-1 ring-amber-500" /></div>
          <div><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Wedstrijden p.p.</span><input type="number" value={matchesPerPlayer} onChange={e => setMatchesPerPlayer(+e.target.value)} className="w-full bg-gray-800 p-3 rounded-xl font-bold border border-gray-700 outline-none focus:ring-1 ring-amber-500" /></div>
          <div className="flex gap-2">{[4, 5].map(n => <button key={n} onClick={() => setPlayersPerTeam(n)} className={`flex-1 py-3 rounded-xl font-black border-2 transition-all ${playersPerTeam === n ? 'bg-amber-500 border-amber-400 text-white shadow-lg' : 'bg-gray-800 border-gray-700 text-gray-500'}`}>{n} vs {n}</button>)}</div>
        </div>
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-white font-bold uppercase text-[10px] tracking-widest">Kies Spelersaantal:</h3>
          <div className="grid sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
            {possibilities.map(opt => (
              <button key={opt.playerCount} onClick={() => {setTargetPlayerCount(opt.playerCount); setSelectedPlayerIds(new Set());}} className={`p-4 rounded-2xl border-2 text-left transition-all ${targetPlayerCount === opt.playerCount ? 'ring-2 ring-white scale-[1.02] shadow-xl' : opt.color}`}>
                <div className="flex justify-between font-black"><div className="text-2xl text-white">{opt.playerCount} Spelers</div><span className="text-[8px] px-2 py-1 rounded-full bg-black/30 text-white uppercase">{opt.label}</span></div>
                <div className="mt-2 text-[9px] font-bold text-gray-400 uppercase tracking-tighter">📅 {opt.totalRounds} Rondes | 🪑 {opt.resting} Rust</div>
              </button>
            ))}
          </div>
          {targetPlayerCount && (
            <div className="pt-4 space-y-4 animate-fade-in border-t border-gray-700 mt-4">
               <textarea value={attendanceText} onChange={e => setAttendanceText(e.target.value)} placeholder="Plak aanwezigheidslijst..." className="w-full h-24 bg-gray-900 border border-gray-700 rounded-xl p-3 text-xs outline-none focus:ring-2 ring-amber-500 text-white" />
               <button onClick={handleParseAttendance} className="w-full py-3 bg-amber-500 text-white font-black rounded-xl uppercase text-[10px] tracking-widest shadow-lg hover:bg-amber-400 transition-colors">Verwerk Namen</button>
               <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 max-h-48 overflow-y-auto p-2 bg-black/20 rounded-xl">
                 {players.map(p => (
                   <button key={p.id} onClick={() => { const n = new Set(selectedPlayerIds); if (n.has(p.id)) n.delete(p.id); else if (n.size < targetPlayerCount!) n.add(p.id); setSelectedPlayerIds(n); }} className={`p-2 rounded-lg text-[10px] font-bold border transition-all ${selectedPlayerIds.has(p.id) ? 'bg-amber-500 border-amber-400 text-white shadow-md' : 'bg-gray-800 border-gray-700 text-gray-500'}`}>{p.name}</button>
                 ))}
               </div>
               {selectedPlayerIds.size === targetPlayerCount && (
                 ...
                 }} className="w-full bg-green-600 text-white font-black py-4 rounded-2xl shadow-xl uppercase hover:bg-green-500 tracking-widest mt-4 animate-pulse">Start Berekening</button>
               )}
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-6 pb-20">
      <style>{`@media print { .no-print { display: none !important; } .match-card { page-break-inside: avoid; border: 3px solid black !important; margin-bottom: 30px; color: black !important; } .match-card * { color: black !important; } }`}</style>
      <div className="no-print flex justify-between items-center bg-gray-800 p-4 rounded-2xl border-b-4 border-amber-500 shadow-xl text-white">
        ...
        <div className="flex bg-gray-900 p-1 rounded-xl">
          {(['schedule', 'standings', 'analysis'] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-2 rounded-lg font-bold text-[10px] uppercase transition-all ${activeTab === t ? 'bg-amber-500 text-white shadow-lg' : 'text-gray-500'}`}>{t === 'schedule' ? 'Schema' : t === 'standings' ? 'Stand' : 'Mix'}</button>
          ))}
        </div>
        <div className="flex gap-2">
          <button onClick={() => window.print()} className="bg-gray-700 px-4 py-2 rounded-lg text-xs font-bold uppercase shadow-md">Print</button>
      ...
      <div className="space-y-12">
        {activeTab === 'schedule' && (
          <>
            <input type="text" placeholder="Speler zoeken in schema..." value={highlightName} onChange={e => setHighlightName(e.target.value)} className="no-print w-full bg-gray-800 p-4 rounded-xl text-white border border-gray-700 outline-none focus:ring-2 ring-amber-500 shadow-inner" />
            {session.rounds.map((round, rIdx) => (
              <div key={rIdx} className="space-y-4">
                <h3 className="text-2xl font-black text-amber-500 uppercase italic border-b-2 border-gray-700 pb-2 tracking-widest">Ronde {round.roundNumber}</h3>
                <div className="grid lg:grid-cols-2 gap-6">
                  {round.matches.map((match, mIdx) => {
                    ...
                    return (
                      <div key={match.id} className={`match-card bg-gray-800 rounded-2xl border ${match.isPlayed ? 'border-green-500/50 shadow-lg shadow-green-500/5' : 'border-gray-700'} overflow-hidden shadow-xl`}>
                        ...
                        <div className="p-5 flex justify-between items-stretch gap-4 text-white">
                          ...
                          <div className="no-print flex flex-col items-center justify-center gap-2">
                            <div className="flex items-center gap-2">
                              <input type="number" value={match.team1Score} onChange={e => { const s = {...session}; s.rounds[rIdx].matches[mIdx].team1Score = +e.target.value; s.rounds[rIdx].matches[mIdx].isPlayed = true; s.standings = calculateStandings(s); setSession({...s}); }} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border-2 border-gray-700 text-white outline-none focus:border-amber-500" />
                              <span className="text-gray-600 font-bold">-</span>
                              <input type="number" value={match.team2Score} onChange={e => { const s = {...session}; s.rounds[rIdx].matches[mIdx].team2Score = +e.target.value; s.rounds[rIdx].matches[mIdx].isPlayed = true; s.standings = calculateStandings(s); setSession({...s}); }} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border-2 border-gray-700 text-white outline-none focus:border-amber-500" />
                            </div>
                            <button onClick={() => { 
                                const s = {...session}; 
                                ...
                                setSession({...s}); 
                            }} className={`text-[8px] font-black px-2 py-1 rounded transition-colors shadow-md ${match.isPlayed ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                          ...
                          <div className="flex-1 text-right flex flex-col justify-between">
                            ...
                            <div className="text-[9px] text-gray-500 mt-2 font-black text-right tracking-tighter">GEM: {avg2.toFixed(2)}</div>
                          </div>
                        </div>
                        <div className="p-2 bg-gray-900/30 border-t border-gray-700 flex justify-center gap-6 text-[9px] font-black uppercase text-pink-400 tracking-tight">
        ...
        {activeTab === 'standings' && (
          <div className="bg-gray-800 rounded-3xl shadow-2xl border border-gray-700 overflow-hidden text-white">
            <table className="w-full text-left">
              <thead className="bg-gray-900 text-gray-400 text-[10px] uppercase font-black tracking-widest shadow-md">
                <tr><th className="p-4 w-12 text-center">#</th><th className="p-4">Deelnemer</th><th className="p-4 text-center">W</th><th className="p-4 text-center">PTN</th><th className="p-4 text-center">DS</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-700 uppercase">
                {session.standings.map((entry, idx) => (
                  <tr key={entry.playerId} className={idx < 3 ? 'bg-amber-500/5' : 'hover:bg-gray-700/20 transition-colors'}>
                    <td className="p-4 font-black text-amber-500 text-center">{idx + 1}</td>
                    <td className="p-4 font-bold text-sm tracking-tight">{entry.playerName}</td>
                    <td className="p-4 text-center text-gray-400 font-bold">{entry.matchesPlayed}</td>
        ...
        {activeTab === 'analysis' && (
          <div className="space-y-4 no-print animate-fade-in text-white">
            <input type="text" placeholder="Filter duo's..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-gray-800 border-gray-700 rounded-xl p-4 text-sm outline-none focus:ring-2 ring-amber-500 shadow-inner" />
            <div className="bg-gray-800 rounded-3xl border border-gray-700 overflow-hidden max-h-[600px] overflow-y-auto uppercase shadow-2xl">
              <table className="w-full text-left">
                <thead className="bg-gray-900 text-gray-400 text-[10px] uppercase font-black sticky top-0 z-10 shadow-md">
                ...
                <tbody className="divide-y divide-gray-700">
                  {coOpData.filter(d => d.p1.toLowerCase().includes(searchTerm.toLowerCase()) || d.p2.toLowerCase().includes(searchTerm.toLowerCase())).map((pair, i) => (
                    <tr key={i} className={pair.together > 1 ? 'bg-red-500/10' : 'hover:bg-gray-700/20 transition-colors'}>
                      <td className="p-4 text-xs font-bold tracking-tight">{pair.p1} + {pair.p2}</td>
