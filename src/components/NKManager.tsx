...
async function generateSingleVersion(
  ...
  while (rIdx <= totalRounds) {
      ...
      try {
        for (let h = 0; h < mInRound; h++) {
          ...
          while (selectedForMatch.length < ppm) {
            ...
            remaining.sort((a, b) => {
              const scoreA = selectedForMatch.reduce(
                (sum, p) =>
                  sum +
                  (pairCounts.get(
                    [p.id, a.id].sort().join('-')
              ...
              const scoreB = selectedForMatch.reduce(
                (sum, p) =>
                  sum +
                  (pairCounts.get(
                    [p.id, b.id].sort().join('-')
          ...
          const diff = Math.abs(
            (
              split.t1.reduce((s, p) => s + p.rating, 0) /
              ppt
            ) -
            (
              split.t2.reduce((s, p) => s + p.rating, 0) /
              ppt
            )
          );
    ...
    if (success) {
      ...
      rounds.push({
        ...
        // De spelers die na wedstrijden, reserves en scheidsrechters
        // nog over zijn, doen deze ronde helemaal niets.
  ...
  const lastCounts =
    playedCountsHistory[playedCountsHistory.length - 1];
...
export async function generateNKSchedule(
  ...
  while (
    ...
    const session = await generateSingleVersion(
      players,
      hallNames,
      mpp,
      ppt,
  ...
  let candidates =
    validVersions.filter(
      v => getMaxDiff(v) <= balanceThreshold
    );

  if (candidates.length === 0) {
    candidates = [...validVersions]
      .sort(
        (a, b) =>
          getMaxDiff(a) -
          getMaxDiff(b)
