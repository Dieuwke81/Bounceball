    ...
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-700 space-y-4">
          <div>
            <span className="text-[10px] text-gray-500 font-black uppercase">Zalen</span>
          ...
          <div>
            <span className="text-[10px] text-gray-500 font-black uppercase">Wedstrijden p.p.</span>
          ...
          <div className="grid sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
            {possibilities.map(opt => (
              <button key={opt.playerCount} onClick={() => {setTargetPlayerCount(opt.playerCount); setSelectedPlayerIds(new Set());}} className={`p-4 rounded-2xl border-2 text-left transition-all ${targetPlayerCount === opt.playerCount ? 'ring-2 ring-white scale-[1.02]' : opt.color}`}>
                ...
                <div className="mt-2 text-[9px] font-bold text-gray-400 uppercase">📅 {opt.totalRounds} Rondes | 🪑 {opt.resting} Rust (Nodig: {opt.needs})</div>
              </button>
            ))}
          </div>
          ...
          {targetPlayerCount && (
            <div className="pt-4 space-y-4 animate-fade-in border-t border-gray-700 mt-4">
               ...
               {selectedPlayerIds.size === targetPlayerCount && (
                 ...
                 }} className="w-full bg-green-600 text-white font-black py-4 rounded-2xl shadow-xl uppercase hover:bg-green-500 tracking-widest mt-4">Genereer Schema</button>
               )}
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="no-print flex justify-between items-center bg-gray-800 p-4 rounded-2xl border-b-4 border-amber-500 shadow-xl">
        <h2 className="text-xl font-black text-white italic uppercase">NK Manager</h2>
      ...
      <div>
        {activeTab === 'schedule' && (
          <div className="space-y-12">
            <input type="text" placeholder="Zoek naam in schema..." value={highlightName} onChange={e => setHighlightName(e.target.value)} className="no-print w-full bg-gray-800 p-4 rounded-xl text-white border border-gray-700 outline-none" />
