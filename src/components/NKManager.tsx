...
const NKManager: React.FC<NKManagerProps> = ({ players, introPlayers = [], onClose }) => {
  ...
  const calculatedOptions = useMemo(() => {
    ...
    for (let mpp = 3; mpp <= 12; mpp++) {
      if ((n * mpp) % pPerMatch === 0) {
        ...
        // NIEUW: Berekening van mensen die écht niks doen (niet ref of reserve zijn)
        const needs = hUsed * 3; // 2 reserves + 1 ref per zaal
        const doingNothing = Math.max(0, resting - needs);
        ...
        options.push({ mpp, totalMatches, rounds, resting, doingNothing, label, color, score });
      }
    }
  ...
  const handleStartFixedNK = async () => {
    setIsGenerating(true); setProgressMsg("Optimaliseren (Best of 1.000.000)...");
    try {
  ...
  if (isGenerating) return (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-white text-center font-black">
      ...
      <h2 className="text-3xl font-black uppercase italic tracking-tighter font-black">{progressMsg}</h2>
      <p className="text-gray-500 text-xs mt-2 uppercase font-bold animate-pulse tracking-widest text-center font-black">Berekeningen worden uitgevoerd...</p>
    </div>
  );
  ...
  if (!session) return (
    <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded-3xl border border-amber-500/20 shadow-2xl space-y-6 text-white font-black">
      ...
      {playerSource === 'database' && (
        <div className="flex bg-gray-900/50 p-1 rounded-2xl w-fit mx-auto border border-gray-700 mt-2 animate-fade-in font-black">
          <button onClick={() => setNkType('individual')} className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase transition-all ${nkType === 'individual' ? 'bg-cyan-500 text-white shadow-lg' : 'text-gray-600 font-black font-black'}`}>Individueel</button>
          <button onClick={() => setNkType('fixed')} className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase transition-all ${nkType === 'fixed' ? 'bg-cyan-500 text-white shadow-lg' : 'text-gray-600 font-black font-black'}`}>Vaste Teams</button>
        </div>
      )}

      {errorAnalysis && (
        <div className="bg-red-500/10 border-2 border-red-500/50 p-4 rounded-2xl text-white font-black">
          <h3 className="text-red-500 font-black uppercase text-sm font-black font-black font-black">Let op: Analyse Mislukking</h3>
          <p className="text-gray-300 text-[10px] mt-1 leading-relaxed whitespace-pre-line font-black font-black">{errorAnalysis}</p>
        </div>
      )}
      <div className="grid lg:grid-cols-3 gap-8 text-white font-black">
        <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-700 space-y-4 text-white font-black">
          <div><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest font-black font-black font-black">Zalen</span><input type="number" value={hallsCount} onFocus={(e) => e.target.select()} onChange={e => handleHallsCountChange(+e.target.value)} className="w-full bg-gray-800 p-3 rounded-xl font-bold border border-gray-700 focus:border-amber-500 outline-none text-white font-black font-black" /></div>
          <div className="space-y-2 text-white font-black font-black font-black font-black">
            <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest text-white font-black font-black">Zaalnamen</span>
            <div className="grid grid-cols-1 gap-2 font-black font-black">
              {hallNames.map((name, i) => (
                <input key={i} type="text" value={name} onChange={(e) => {
                  ...
                  }} className="bg-gray-800 p-2 rounded-lg text-xs font-bold border border-gray-700 focus:border-amber-500 outline-none text-white font-black uppercase font-black" placeholder={`Zaal ${i+1}`}
                />
              ))}
            </div>
          </div>
          <div><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest font-black font-black font-black">Min. Team Rating</span><input type="number" step="0.01" value={minTeamRating} onFocus={(e) => e.target.select()} onChange={e => setMinTeamRating(+e.target.value)} className="w-full bg-gray-800 p-3 rounded-xl font-bold border border-gray-700 focus:border-amber-500 outline-none text-white font-black font-black" /></div>
          <div className="flex gap-2 text-white font-black font-black font-black font-black font-black">{[4, 5].map(n => <button key={n} onClick={() => setPlayersPerTeam(n)} className={`flex-1 py-3 rounded-xl font-black border-2 ${playersPerTeam === n ? 'bg-amber-500 border-amber-400 text-white' : 'bg-gray-800 border-gray-700 text-gray-500'}`}>{n} vs {n}</button>)}</div>
          <textarea value={attendanceText} onChange={e => setAttendanceText(e.target.value)} placeholder="Plak WhatsApp lijst..." className="w-full h-40 bg-gray-900 border border-gray-700 rounded-xl p-3 text-xs outline-none text-white font-black font-black font-black font-black" />
          <button onClick={handleParseAttendance} className="w-full py-3 bg-amber-500 text-white font-black rounded-xl uppercase text-xs hover:bg-amber-400 transition-all tracking-widest shadow-lg font-black font-black font-black">Verwerk Lijst</button>
        </div>
        <div className="lg:col-span-2 space-y-4 text-white font-black">
          <div className="flex justify-between items-end text-white font-black font-black font-black"><h3 className="text-white font-bold uppercase text-xs tracking-widest font-black font-black font-black">Deelnemers ({selectedPlayerIds.size})</h3><button onClick={() => setSelectedPlayerIds(new Set())} className="text-[10px] text-red-500 font-bold uppercase underline font-black">Wis alles</button></div>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 max-h-48 overflow-y-auto p-2 bg-black/20 rounded-xl custom-scrollbar font-black font-black">
            {activePlayerPool.map(p => {
                ...
                return (
                  <button 
                    ...
                    className={`p-2 rounded-lg text-[10px] font-black border transition-all truncate font-black
                        ${selectedPlayerIds.has(p.id) 
                            ? (playerSource === 'intro' ? (rConfig?.bg || 'bg-amber-500') : 'bg-amber-500') + ' border-white text-white font-black font-black' 
                            : 'bg-gray-800 border-gray-700 font-black'}`}
                  >
                    <span className={!selectedPlayerIds.has(p.id) && playerSource === 'intro' ? (rConfig?.text || 'text-white') : 'text-white font-black font-black font-black'}>
                        {p.name}
                    </span>
                  </button>
                );
          ...
          {(nkType === 'individual' || playerSource === 'intro') ? (
            <div className="pt-4 border-t border-gray-700 space-y-4 font-black">
              <h3 className="text-white font-bold uppercase text-xs mb-3 text-amber-500 tracking-widest font-black font-black font-black">1. Kies Toernooivorm:</h3>
              <div className="grid sm:grid-cols-2 gap-3 font-black">
                {calculatedOptions.map(opt => (
                  <button key={opt.mpp} onClick={() => handleSelectOption(opt)} className={`p-4 rounded-2xl border-2 text-left transition-all ${selectedOption?.mpp === opt.mpp ? 'border-amber-500 bg-amber-500/20' : 'border-gray-700 bg-gray-800/40 hover:scale-[1.02]'} font-black`}>
                    <div className="text-xl font-black tracking-tighter text-white font-black font-black">{opt.mpp} Wedstrijden p.p.</div>
                    <div className="mt-1 text-[9px] font-bold text-gray-400 uppercase tracking-wider text-white font-black font-black font-black">
                        {opt.rounds} rondes | {opt.resting} rust ({opt.doingNothing} doen niks) | {opt.label}
                    </div>
                  </button>
                ))}
              </div>
              {selectedOption && (
                <div className="bg-gray-900/50 p-6 rounded-3xl border-2 border-amber-500/30 animate-fade-in space-y-4 font-black font-black">
                  <h3 className="text-white font-bold uppercase text-xs text-amber-500 tracking-widest text-center font-black font-black font-black">2. Voer Tijdschema in (Handmatig):</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-64 overflow-y-auto p-2 custom-scrollbar font-black font-black">
                    {manualTimes.map((time, idx) => (
                      <div key={idx} className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-white text-center font-black font-black">
                        <div className="text-[10px] text-gray-500 font-black mb-2 uppercase font-black font-black font-black">Ronde {idx + 1}</div>
                        <div className="flex items-center gap-1 font-black">
                          <input type="text" value={time.start} placeholder="10:00" onChange={e => updateManualTime(idx, 'start', e.target.value)} className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black font-black font-black font-black" />
                          <span className="text-gray-600 font-black font-black">-</span>
                          <input type="text" value={time.end} placeholder="10:20" onChange={e => updateManualTime(idx, 'end', e.target.value)} className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black font-black font-black font-black" />
                        </div>
                      </div>
                    ))}
                  </div>
                  <button onClick={async () => {
                    ...
                    }} className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl uppercase tracking-widest shadow-xl transition-all font-black font-black font-black font-black font-black">Start Toernooi</button>
                </div>
              )}
            </div>
          ) : (
            <div className="pt-4 border-t border-gray-700 space-y-4 text-white font-black font-black font-black">
               <div className="bg-amber-500/10 p-6 rounded-3xl border-2 border-amber-500/30 text-center animate-fade-in space-y-2 font-black font-black font-black">
                <p className="text-sm font-black uppercase text-amber-500 font-black font-black font-black font-black">Vaste teams van {playersPerTeam}</p>
                <p className="text-[10px] text-gray-400 uppercase font-bold font-black font-black font-black font-black font-black font-black">Iedereen speelt 1x tegen elk ander team (Round Robin).</p>
                {selectedPlayerIds.size > 0 && selectedPlayerIds.size % playersPerTeam !== 0 && (
                    <p className="text-red-500 text-[10px] mt-2 font-black italic font-black font-black font-black font-black font-black">LET OP: Selecteer nog {playersPerTeam - (selectedPlayerIds.size % playersPerTeam)} spelers (veelvoud van {playersPerTeam} nodig).</p>
                )}
                {selectedPlayerIds.size > 0 && selectedPlayerIds.size % playersPerTeam === 0 && (
                    <button onClick={() => {
                    ...
                    }} className="mt-4 bg-amber-500 text-white px-8 py-3 rounded-2xl text-xs font-black uppercase shadow-lg hover:scale-105 transition-all font-black font-black font-black font-black">Bevestig Deelnemers</button>
                )}
               </div>
               {manualTimes.length > 0 && (
                <div className="bg-gray-900/50 p-6 rounded-3xl border-2 border-amber-500/30 animate-fade-in space-y-4 font-black font-black font-black font-black">
                  <h3 className="text-white font-bold uppercase text-xs text-amber-500 tracking-widest text-center font-black font-black font-black font-black">Voer Tijdschema in (Handmatig):</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-64 overflow-y-auto p-2 custom-scrollbar font-black font-black font-black font-black">
                    {manualTimes.map((time, idx) => (
                      <div key={idx} className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-white text-center font-black font-black font-black">
                        <div className="text-[10px] text-gray-500 font-black mb-2 uppercase font-black font-black font-black font-black">Ronde {idx + 1}</div>
                        <div className="flex items-center gap-1 font-black font-black">
                          <input type="text" value={time.start} placeholder="10:00" onChange={e => { const n = [...manualTimes]; n[idx].start = e.target.value; setManualTimes(n); }} className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black font-black font-black font-black font-black" />
                          <span className="text-gray-600 font-black font-black font-black font-black font-black font-black">-</span>
                          <input type="text" value={time.end} placeholder="10:20" onChange={e => { const n = [...manualTimes]; n[idx].end = e.target.value; setManualTimes(n); }} className="w-full bg-gray-900 text-[10px] p-1.5 rounded border border-gray-700 text-white text-center font-black font-black font-black font-black font-black" />
                        </div>
                      </div>
                    ))}
                  </div>
                  <button onClick={handleStartFixedNK} className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl uppercase tracking-widest shadow-xl transition-all font-black font-black font-black font-black font-black font-black">Start Vaste Teams Toernooi</button>
                </div>
               )}
            </div>
          )}
  ...
  return (
    <div className="space-y-6 pb-20 text-white font-black font-black font-black font-black">
      <style>{`
        @media print {
      ...
      {printMenuOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 no-print text-white font-black font-black font-black">
          <div className="bg-gray-800 border-2 border-amber-500 rounded-3xl p-8 max-w-sm w-full space-y-6 shadow-2xl text-white font-black font-black font-black font-black">
            <h3 className="text-2xl font-black uppercase italic text-center text-white font-black font-black font-black">Print Menu</h3>
            <div className="space-y-3 font-black font-black font-black">
              <button onClick={() => handlePrintAction('overview')} className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black font-black font-black">Compleet Overzicht</button>
              <button onClick={() => handlePrintAction('halls')} className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black font-black font-black">Per Zaal</button>
              <button onClick={() => handlePrintAction('players')} className="w-full py-4 bg-gray-700 hover:bg-amber-500 text-white font-bold rounded-2xl transition-all uppercase text-xs font-black font-black font-black">Individuele Spelers</button>
            </div>
            <button onClick={() => setPrintMenuOpen(false)} className="w-full text-gray-500 font-bold uppercase text-[10px] hover:text-white transition-colors font-black font-black font-black">Annuleren</button>
          </div>
        </div>
      )}

      <div className="no-print bg-gray-800 p-6 rounded-3xl border-b-8 border-amber-500 shadow-2xl text-white space-y-6 font-black font-black font-black font-black font-black">
        <h2 className="text-4xl sm:text-6xl font-black italic uppercase tracking-tighter text-center font-black font-black font-black font-black">NK Manager</h2>
        <div className="flex justify-center text-white font-black font-black font-black font-black">
          <div className="flex bg-gray-900 p-1.5 rounded-2xl gap-1 w-full max-w-md font-black font-black font-black font-black font-black">
            {(['schedule', 'standings', 'analysis'] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${activeTab === t ? 'bg-amber-500 text-white shadow-lg font-black font-black font-black' : 'text-gray-500 hover:text-gray-300 font-black font-black font-black font-black font-black'}`}>{t === 'schedule' ? 'Schema' : t === 'standings' ? 'Stand' : 'Check'}</button>
            ))}
          </div>
        </div>
      ...
      <div className="no-print space-y-8 text-white font-black font-black font-black font-black">
        {activeTab === 'schedule' && (
          <>
            <input type="text" placeholder="Naam markeren..." value={highlightName} onChange={e => setHighlightName(e.target.value)} className="w-full bg-gray-800 p-4 rounded-2xl text-white border border-gray-700 outline-none focus:ring-2 ring-green-500 transition-all font-black uppercase font-black font-black font-black" />
            
            {(session as any).isFixedTeams && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 animate-fade-in font-black font-black font-black font-black">
                    <div className="col-span-full text-center text-gray-500 text-[10px] uppercase font-black mb-2 font-black font-black font-black font-black">Vaste Teams (Groepsgemiddelde):</div>
                    {(session as any).fixedTeams.map((team: any, i: number) => (
                        <div key={i} className="bg-gray-800 p-3 rounded-2xl border border-gray-700 shadow-lg font-black font-black font-black font-black">
                            <div className="flex justify-between items-center mb-1 font-black font-black font-black">
                                <input 
                                    type="text" 
                                    value={team.name} 
                                    onChange={e => updateFixedTeamName(i, e.target.value)} 
                                    className="bg-transparent text-amber-500 text-xs font-black uppercase outline-none focus:ring-1 ring-amber-500 rounded px-1 w-2/3 font-black font-black font-black font-black font-black" 
                                />
                                <span className="text-[9px] text-gray-500 font-mono font-black font-black font-black font-black font-black">avg: {team.avgRating.toFixed(2)}</span>
                            </div>
            ...
            {! (session as any).isFixedTeams && (
                <div className="bg-gray-800/80 border-l-4 border-amber-500 p-4 rounded-r-2xl flex items-center gap-6 shadow-xl mb-4 text-white font-black font-black font-black font-black font-black">
                <div className="flex-1 flex justify-between items-center text-white font-black font-black font-black">
                    <div>
                    ...
                    <div className="flex items-baseline gap-2 mt-1 font-black font-black font-black font-black font-black">
            ...
            {session.rounds.map((round, rIdx) => (
              <div key={rIdx} className="space-y-4 text-white font-black font-black font-black font-black">
                <div className="flex items-center gap-4 text-white font-black font-black font-black font-black">
                    <h3 className="text-xl font-black text-amber-500 uppercase italic border-l-4 border-amber-500 pl-4 tracking-tighter text-white font-black font-black font-black font-black">Ronde {round.roundNumber}</h3>
                    {(round as any).startTime && (
                        <span className="bg-gray-800 px-3 py-1 rounded-lg border border-gray-700 text-xs font-black text-gray-400 font-black font-black font-black font-black font-black">
                            {(round as any).startTime} - {(round as any).endTime}
                        </span>
                    )}
                </div>
                <div className="grid lg:grid-cols-2 gap-6 text-white font-black font-black font-black font-black font-black">
                  {round.matches.map((match, mIdx) => {
                    const isFixed = (session as any).isFixedTeams;
                    let t1A = match.team1; let t2A = match.team2;
                    if (isFixed) { if ((match as any).t1ReserveId) t1A = match.team1.filter(p => p.id !== (match as any).t1ReserveId); if ((match as any).t2ReserveId) t2A = match.team2.filter(p => p.id !== (match as any).t2ReserveId); }
                    const avg1 = t1A.reduce((s, p) => s + p.rating, 0) / t1A.length; const avg2 = t2A.reduce((s, p) => s + p.rating, 0) / t2A.length;

                    return (
                      <div key={match.id} className={`match-card bg-gray-800 rounded-2xl border-2 ${match.isPlayed ? 'border-green-500/50 shadow-lg shadow-green-500/5 font-black font-black font-black font-black' : 'border-gray-700 font-black font-black'} overflow-hidden text-white font-black font-black font-black`}>
                        <div className="bg-gray-900/50 p-3 flex justify-between text-[10px] font-black uppercase text-gray-500 tracking-widest text-white font-black font-black font-black font-black font-black">
                          <span>📍 ZAAL <span className="text-red-500 text-sm ml-1 font-black uppercase text-white font-black font-black font-black font-black font-black">{match.hallName}</span></span>
                          {isFixed ? (
                             <span className="text-cyan-400 font-black font-black font-black font-black font-black">Vaste Teams</span>
                          ) : (
                             <span className={`px-2 py-0.5 rounded transition-all ${isHighlighted(match.referee?.name || '') ? 'bg-green-500 text-white font-black scale-110 shadow-lg font-black font-black font-black font-black' : 'text-pink-400 font-black font-black font-black'}`}>Ref: {match.referee?.name}</span>
                          )}
                        </div>
                        <div className="p-5 flex justify-between items-stretch gap-4 text-white font-black font-black font-black font-black font-black font-black font-black">
                          <div className="flex-1 space-y-1 text-left text-white font-black font-black">
                            <div className="text-[9px] text-blue-400 font-black uppercase mb-2 tracking-widest text-white font-black font-black font-black font-black font-black font-black font-black font-black font-black">{(match as any).team1Name || 'Team Blauw'}</div>
                            {match.team1.map(p => {
                            ...
                            <div className="text-[9px] text-gray-500 mt-2 font-black font-black font-black font-black font-black font-black font-black">GEM: {avg1.toFixed(2)}</div>
                          </div>
                          <div className="flex flex-col items-center justify-center gap-3 text-white font-black font-black font-black font-black font-black">
                            <div className="flex items-center gap-2 text-white font-black font-black font-black font-black font-black font-black font-black font-black font-black">
                              <input type="number" value={match.team1Score} onFocus={(e) => e.target.select()} onChange={e => { const n = JSON.parse(JSON.stringify(session)); n.rounds[rIdx].matches[mIdx].team1Score = +e.target.value; n.rounds[rIdx].matches[mIdx].isPlayed = true; setSession(n); }} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border border-gray-700 text-white outline-none focus:border-amber-500 font-black font-black font-black font-black font-black" />
                              <span className="text-gray-600 font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">-</span>
                              <input type="number" value={match.team2Score} onFocus={(e) => e.target.select()} onChange={e => { const n = JSON.parse(JSON.stringify(session)); n.rounds[rIdx].matches[mIdx].team2Score = +e.target.value; n.rounds[rIdx].matches[mIdx].isPlayed = true; setSession(n); }} className="w-12 h-12 bg-gray-900 text-center rounded-xl font-black text-xl border border-gray-700 text-white outline-none focus:border-amber-500 font-black font-black font-black font-black font-black" />
                            </div>
                            <div className="flex flex-col items-center gap-1 text-white font-black font-black font-black font-black font-black font-black font-black font-black">
                              <button onClick={() => { const n = JSON.parse(JSON.stringify(session)); n.rounds[rIdx].matches[mIdx].isPlayed = true; setSession(n); }} className={`text-[8px] font-black px-3 py-1.5 rounded-lg transition-all ${match.isPlayed ? 'bg-green-600 text-white shadow-lg font-black font-black font-black font-black font-black' : 'bg-gray-700 text-gray-400 hover:text-white font-black'}`}>{match.isPlayed ? 'VERWERKT' : 'OPSLAAN'}</button>
                              <button onClick={() => { const n = JSON.parse(JSON.stringify(session)); n.rounds[rIdx].matches[mIdx].isPlayed = false; n.rounds[rIdx].matches[mIdx].team1Score = 0; n.rounds[rIdx].matches[mIdx].team2Score = 0; setSession(n); }} className="text-[8px] text-red-500 font-black mt-1 uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">Reset</button>
                              <div className="text-[8px] text-gray-500 font-bold uppercase tracking-tighter mt-1 font-black font-black font-black font-black font-black font-black font-black font-black font-black">Verschil: {Math.abs(avg1 - avg2).toFixed(2)}</div>
                            </div>
                          </div>
                          <div className="flex-1 space-y-1 text-right text-white font-black font-black font-black font-black font-black">
                            <div className="text-[9px] text-amber-400 font-black uppercase mb-2 tracking-widest text-white font-black font-black font-black font-black font-black font-black font-black font-black">{(match as any).team2Name || 'Team Geel'}</div>
                            {match.team2.map(p => {
                                const rColor = ratingColors.get(p.rating);
                            ...
                            <div className="text-[9px] text-gray-500 mt-2 font-black font-black font-black font-black font-black font-black font-black font-black">GEM: {avg2.toFixed(2)}</div>
                          </div>
                        </div>
          ...
          <div className="bg-gray-800 rounded-3xl shadow-2xl border border-gray-700 overflow-hidden text-white animate-fade-in font-black text-center font-black font-black font-black font-black">
            <table className="w-full text-left font-black text-white uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">
              <thead className="bg-gray-900 text-gray-400 text-[10px] font-black tracking-widest uppercase font-black font-black font-black font-black font-black">
                <tr><th className="p-5 w-12 text-center text-white font-black uppercase font-black font-black">#</th><th className="p-5 text-white font-black uppercase text-left font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">{(session as any).isFixedTeams ? 'Team' : 'Speler'}</th><th className="p-5 text-center text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">W</th><th className="p-5 text-center text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">DS</th><th className="p-5 text-center text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">PTN</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-700/50 uppercase text-white font-black font-black font-black font-black font-black">
                {currentStandings.map((entry, idx) => (
                  <tr key={idx} className={`transition-colors ${idx < 3 ? 'bg-amber-500/5' : 'hover:bg-gray-700/30'} text-white font-black font-black font-black font-black font-black`}>
                    ...
                    <td className="p-5 text-center text-gray-400 font-mono text-xs text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">{(entry as any).matchesPlayed}</td>
            ...
            <div className="bg-gray-800 rounded-3xl border border-gray-700 overflow-hidden max-h-[600px] overflow-y-auto uppercase custom-scrollbar text-white text-center font-black font-black font-black font-black font-black font-black font-black font-black">
              <table className="w-full text-left text-white font-black uppercase text-center font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">
                <thead className="bg-gray-900 text-gray-400 text-[10px] uppercase font-black sticky top-0 z-20 text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">
                  <tr><th className="p-5 text-white font-black uppercase text-left font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">Duo</th><th className="p-5 text-center text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">Samen</th><th className="p-5 text-center text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">Tegen</th><th className="p-5 text-center text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">Totaal</th></tr>
                </thead>
                  ...
                  {coOpData.filter(d => d.p1.toLowerCase().includes(searchTerm.toLowerCase()) || d.p2.toLowerCase().includes(searchTerm.toLowerCase())).map((pair, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-gray-700/20 transition-colors text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">
                        ...
                        <td className="p-5 text-center text-xs text-gray-400 font-mono text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">{pair.together}x</td>
                        <td className="p-5 text-center text-xs text-gray-400 font-mono text-white font-black uppercase font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black">{pair.against}x</td>
                        <td className="p-5 text-center font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black font-black"><span className={`px-3 py-1 rounded-lg text-[10px] font-black transition-all ${totalColor}`}>{total}x</span></td>
                      </tr>
                    );
                  })}
                </tbody>
