...
const Statistics: React.FC<StatisticsProps> = ({ history, players, onSelectPlayer }) => {
  ...
  const topScorers = useMemo(() => {
    ...
    history.forEach((session) => {
      const allMatches = [
        ...(session.round1Results || []),
        ...(session.round2Results || []),
      ];
  ...
  // ============================
  // Competitie punten (punten / wedstrijden)
  // ============================
  ...
  const attendanceHistory = useMemo(() => {
    ...
    return history
      .map((session) => ({
        date: session.date,
        count: (getTeamsR1(session).flat().length || 0), // trend: aanwezigen op avond (R1 teams is meestal “de opkomst”)
      }))
  ...
  const StatRow: React.FC<{
    ...
    >
      <div className="flex items-center min-w-0">
        ...
        <span className={`ml-3 font-medium truncate ${meetsThreshold ? 'text-gray-200' : 'text-red-400'}`}>
          {player.name}
        </span>
        {player.isFixedMember && (
          <span className="ml-2 text-xs font-semibold bg-green-500 text-white py-0.5 px-2 rounded-full" title="Vast lid">
            Lid
          </span>
        )}
        {player.isKeeper && (
          <span className="ml-2 text-xs font-semibold bg-amber-500 text-white py-0.5 px-2 rounded-full" title="Keeper">
            K
          </span>
        )}
      </div>
  ...
  const AttendanceChart: React.FC<{ data: { date: string; count: number }[] }> = ({ data }) => {
    if (data.length < 2) return <p className="text-gray-400 text-center py-8">Niet genoeg data voor een grafiek.</p>;
  ...
  const SmallToggle: React.FC<{ checked: boolean; onChange: () => void; label: string }> = ({ checked, onChange, label }) => (
      ...
      <div className="text-center mb-4">
        <p className="text-gray-400">
          ...
          <span className="italic">
            Voor de ranglijsten (gem.) moet een speler minimaal <span className="font-bold text-white">{minGames}</span> keer aanwezig zijn geweest.
          </span>
        </p>
