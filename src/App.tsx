...
const App: React.FC = () => {
  ...
  const handleSaveRound1 = (matches: Match[]) => {
    ...
    const points = teams.map((_, i) => ({ teamIndex: i, points: 0, gd: 0, gf: 0 }));
    results.forEach((r) => {
      const s1 = r.team1Goals.reduce((s, g) => s + Number(g.count), 0);
      const s2 = r.team2Goals.reduce((s, g) => s + Number(g.count), 0);
      const t1 = points.find(t => t.teamIndex === r.team1Index)!;
      const t2 = points.find(t => t.teamIndex === r.team2Index)!;
      t1.gd += s1 - s2; t1.gf += s1; t2.gd += s2 - s1; t2.gf += s2;
      if (s1 > s2) t1.points += 3; else if (s2 > s1) t2.points += 3; else { t1.points += 1; t2.points += 1; }
    });
    points.sort((a, b) => b.points - a.points || b.gd - a.gd || b.gf - a.gf);
    const pairings: Match[] = []; const avail = [...points];
    while (avail.length) {
  ...
  const handleRegenerateTeamsForR2 = async () => {
    ...
    try {
      let reg = await generateTeams(attendingPlayers, originalTeams.length, constraints, null, activeHistory);
      reg = optimizeTeamsSoft({
        teams: reg, constraints, attendingIds: new Set(attendingPlayers.map(p => p.id)), seasonPairCounts,
        separateFrequent: separateFrequentTeammates, pairInfrequent: pairInfrequentTeammates,
        separateTop6: separateTop6OnPoints, top6Ids
      });
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'playerDetail': return selectedPlayer ? <PlayerDetail player={selectedPlayer} history={activeHistory} players={players} ratingLogs={ratingLogs} trophies={trophies} seasonStartDate={seasonStartDate} competitionName={competitionName} onBack={() => setCurrentView('stats')} /> : <div className="text-center p-8 bg-gray-800 rounded-xl"><p>Niet gevonden.</p></div>;
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center text-white"><p className="animate-pulse">Laden...</p></div>;
  ...
  return (
    <div className="min-h-screen text-white pb-8">
      <div className="container mx-auto p-4 md:p-6">
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 mb-8 border border-gray-700/50">
